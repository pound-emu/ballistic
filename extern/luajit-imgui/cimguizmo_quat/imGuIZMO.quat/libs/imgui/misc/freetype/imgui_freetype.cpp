...
//  2024/10/17: added plutosvg support for SVG Fonts (seems faster/better than lunasvg). Enable by
//  using '#define IMGUI_ENABLE_FREETYPE_PLUTOSVG'. (#7927) 2023/11/13: added support for
//  ImFontConfig::RasterizationDensity field for scaling render density without scaling metrics.
//  2023/08/01: added support for SVG fonts, enable by using '#define
//  IMGUI_ENABLE_FREETYPE_LUNASVG'. (#6591) 2023/01/04: fixed a packing issue which in some
                       ...
                       unsigned int        extra_font_builder_flags)
{
    FT_Error error = FT_New_Memory_Face(ft_library,
                                        (uint8_t *)src.FontData,
                                        (uint32_t)src.FontDataSize,
                                        (uint32_t)src.FontNo,
                                        &Face);
    if (error != 0)
    {
                ...
                for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
                {
                    for (uint32_t x = 0; x < w; x++)
                    {
                        uint8_t r = src[x * 4 + 2], g = src[x * 4 + 1], b = src[x * 4],
                                a = src[x * 4 + 3];
                        dst[x]
                            = IM_COL32(DE_MULTIPLY(r, a), DE_MULTIPLY(g, a), DE_MULTIPLY(b, a), a);
                    }
                }
            }
            else
...
bool
ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas *atlas, unsigned int extra_flags)
{
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++)
        {
            ...
            IM_ASSERT(buf_bitmap_current_used_bytes + bitmap_size_in_bytes
                      <= BITMAP_BUFFERS_CHUNK_SIZE); // We could probably allocate custom-sized
                                                     // buffer instead.
...
static void *
FreeType_Realloc(FT_Memory /*memory*/, long cur_size, long new_size, void *block)
{
...
void
ImGuiFreeType::SetAllocatorFunctions(void *(*alloc_func)(size_t sz, void *user_data),
...
ImGuiLunasvgPortRender(FT_GlyphSlot slot, FT_Pointer *_state)
{
    ...
    lunasvg::Bitmap bitmap(
        (uint8_t *)slot->bitmap.buffer, slot->bitmap.width, slot->bitmap.rows, slot->bitmap.pitch);
    state->svg->setMatrix(
