...
// Main code
static void
handleAppCmd(struct android_app *app, int32_t appCmd)
{
    switch (appCmd)
    {
...
static int32_t
handleInputEvent(struct android_app *app, AInputEvent *inputEvent)
{
...
android_main(struct android_app *app)
{
    ...
    while (true)
    {
        ...
        while (ALooper_pollOnce(g_Initialized ? 0 : -1, nullptr, &out_events, (void **)&out_data)
               >= 0)
        {
            // Process one event
            if (out_data != nullptr)
...
Init(struct android_app *app)
{
    ...
    // This is mostly boilerplate code for EGL...
    {
        ...
        {
            __android_log_print(
                ANDROID_LOG_ERROR, g_LogTag, "%s", "eglInitialize() returned with an error");
        }
        ...
        {
            __android_log_print(
                ANDROID_LOG_ERROR, g_LogTag, "%s", "eglChooseConfig() returned with an error");
        }
        if (num_configs == 0)
        {
            __android_log_print(
                ANDROID_LOG_ERROR, g_LogTag, "%s", "eglChooseConfig() returned 0 matching config");
        }
        ...
        {
            __android_log_print(
                ANDROID_LOG_ERROR, g_LogTag, "%s", "eglCreateContext() returned EGL_NO_CONTEXT");
        }
    ...
    // We load the default font with increased size to improve readability on many devices with
    // "high" DPI.
...
static int
GetAssetData(const char *filename, void **outData)
{
    int     num_bytes = 0;
