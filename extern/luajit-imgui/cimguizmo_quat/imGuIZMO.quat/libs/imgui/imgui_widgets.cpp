...
bool
ImGui::ButtonEx(const char *label, const ImVec2 &size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
    {
...
bool
ImGui::InvisibleButton(const char *str_id, const ImVec2 &size_arg, ImGuiButtonFlags flags)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = GetCurrentWindow();
    if (window->SkipItems)
...
bool
ImGui::ArrowButtonEx(const char *str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = GetCurrentWindow();
    if (window->SkipItems)
                   ...
                   ImDrawFlags   draw_rounding_flags)
{
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        else
        {
                ...
                && held_dir == g.ScrollbarSeekMode)
            {
                ...
                *p_scroll_v    = ImClamp(
                    *p_scroll_v + (ImS64)(page_dir * size_visible_v), (ImS64)0, scroll_max);
            }
        }
    ...
    window->DrawList->AddRectFilled(
        bb_frame.Min, bb_frame.Max, bg_col, window->WindowRounding, draw_rounding_flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
    {
...
void
ImGui::ImageWithBg(ImTextureID   user_texture_id,
                   ...
                   const ImVec4 &tint_col)
{
    ...
    window->DrawList->AddImage(
        user_texture_id, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColorU32(tint_col));
}
                   ...
                   const ImVec4 &tint_col)
{
    ...
    return ImageButtonEx(
        window->GetID(str_id), user_texture_id, image_size, uv0, uv1, bg_col, tint_col);
}
...
ImGui::RadioButton(const char *label, bool active)
{
    ...
    if (style.FrameBorderSize > 0.0f)
    {
        ...
        window->DrawList->AddCircle(
            center, radius, GetColorU32(ImGuiCol_Border), num_segment, style.FrameBorderSize);
    }
...
bool
ImGui::TextLink(const char *label)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
    {
...
void
ImGui::TextLinkOpenURL(const char *label, const char *url)
{
    ImGuiContext &g = *GImGui;
    if (url == NULL)
    {
...
void
ImGui::SeparatorTextEx(ImGuiID id, const char *label, const char *label_end, float extra_w)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = g.CurrentWindow;
...
bool
ImGui::BeginCombo(const char *label, const char *preview_value, ImGuiComboFlags flags)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = GetCurrentWindow();
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        RenderTextClipped(
            bb.Min + style.FramePadding, ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    }
    if (label_size.x > 0)
    {
...
bool
ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect &bb, ImGuiComboFlags flags)
{
    ImGuiContext &g = *GImGui;
...
int
ImGui::DataTypeCompare(ImGuiDataType data_type, const void *arg_1, const void *arg_2)
{
    switch (data_type)
    {
        case ImGuiDataType_S8:
...
bool
ImGui::DataTypeClamp(ImGuiDataType data_type, void *p_data, const void *p_min, const void *p_max)
{
    switch (data_type)
    {
        case ImGuiDataType_S8:
...
bool
ImGui::DataTypeIsZero(ImGuiDataType data_type, const void *p_data)
{
    ImGuiContext &g = *GImGui;
...
TYPE
ImGui::RoundScalarWithFormatT(const char *format, ImGuiDataType data_type, TYPE v)
{
    IM_UNUSED(data_type);
...
void
ImParseFormatSanitizeForPrinting(const char *fmt_in, char *fmt_out, size_t fmt_out_size)
{
...
const char *
ImParseFormatSanitizeForScanning(const char *fmt_in, char *fmt_out, size_t fmt_out_size)
{
    const char *fmt_end       = ImParseFormatFindEnd(fmt_in);
...
void
ImGui::SetNextItemRefVal(ImGuiDataType data_type, void *p_data)
{
    ImGuiContext &g = *GImGui;
...
bool
ImGui::InputFloat2(const char *label, float v[2], const char *format, ImGuiInputTextFlags flags)
{
    return InputScalarN(label, ImGuiDataType_Float, v, 2, NULL, NULL, format, flags);
}

bool
ImGui::InputFloat3(const char *label, float v[3], const char *format, ImGuiInputTextFlags flags)
{
    return InputScalarN(label, ImGuiDataType_Float, v, 3, NULL, NULL, format, flags);
}

bool
ImGui::InputFloat4(const char *label, float v[4], const char *format, ImGuiInputTextFlags flags)
{
...
static int
InputTextCalcTextLenAndLineCount(const char *text_begin, const char **out_text_end)
{
    int         line_count = 0;
...
static ImVec2
InputTextCalcTextSize(ImGuiContext *ctx,
                      const char   *text_begin,
                      const char   *text_end,
...
static float
STB_TEXTEDIT_GETWIDTH(ImGuiInputTextState *obj, int line_start_idx, int char_idx)
{
    unsigned int c;
...
static void
STB_TEXTEDIT_LAYOUTROW(StbTexteditRow *r, ImGuiInputTextState *obj, int line_start_idx)
{
...
static int
IMSTB_TEXTEDIT_GETNEXTCHARINDEX_IMPL(ImGuiInputTextState *obj, int idx)
{
    if (idx >= obj->TextLen)
    {
...
static int
IMSTB_TEXTEDIT_GETPREVCHARINDEX_IMPL(ImGuiInputTextState *obj, int idx)
{
    if (idx <= 0)
    {
        return -1;
...
static bool
ImCharIsSeparatorW(unsigned int c)
{
...
static bool
STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState *obj, int pos, const char *new_text, int new_text_len)
{
...
void
ImGuiInputTextState::OnCharPressed(unsigned int c)
{
...
bool
ImGuiInputTextState::HasSelection() const
{
...
int
ImGuiInputTextState::GetCursorPos() const
{
    return Stb->cursor;
}
int
ImGuiInputTextState::GetSelectionStart() const
{
    return Stb->select_start;
}
int
ImGuiInputTextState::GetSelectionEnd() const
{
...
void
ImGuiInputTextCallbackData::InsertChars(int pos, const char *new_text, const char *new_text_end)
{
    // Accept null ranges
...
// buffer directly.
static void
InputTextReconcileUndoState(ImGuiInputTextState *state,
                            const char          *old_buf,
                            int                  old_length,
                            const char          *new_buf,
                            int                  new_length)
                   ...
                   void                  *callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        bool child_visible = BeginChildEx(
            label, id, frame_bb.GetSize(), ImGuiChildFlags_Borders, ImGuiWindowFlags_NoMove);
    ...
    bool hovered = ItemHoverable(
        frame_bb, id, g.LastItemData.ItemFlags | ImGuiItemFlags_NoNavDisableMouseHover);
    if (hovered)
    {
            ...
            if (const char *clipboard = GetClipboardText())
            {
                ...
                for (const char *s = clipboard; *s != 0;)
                {
                    ...
                    if (!InputTextFilterCharacter(
                            &g, &c, flags, callback, callback_user_data, true))
                    {
                        continue;
                    }
                    char c_utf8[5];
                    ...
                    > 0) // If everything was filtered, ignore the pasting operation
                {
                    ...
                    stb_textedit_paste(
                        state, state->Stb, clipboard_filtered.Data, clipboard_filtered.Size - 1);
                    state->CursorFollow = true;
                }
            }
        }
        ...
        if (revert_edit && !is_readonly)
        {
            ...
            else if (strcmp(buf, state->TextToRevertTo.Data) != 0)
            {
                ...
                stb_textedit_replace(
                    state, state->Stb, state->TextToRevertTo.Data, state->TextToRevertTo.Size - 1);
            }
        }
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            const ImVec2 draw_scroll = /*state ? ImVec2(state->Scroll.x, 0.0f) :*/ ImVec2(
                    ...
                    const float        *ref_col)
{
    ...
    if (value_changed)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            ColorConvertRGBtoHSV(R, G, B, H, S, V);
            ColorEditRestoreHS(
                col, &H, &S, &V); // Fix local Hue as display below will use it immediately.
        }
        else if (flags & ImGuiColorEditFlags_InputHSV)
        {
            H = col[0];
    ...
    draw_list->AddCircleFilled(
        sv_cursor_pos, sv_cursor_rad, user_col32_striped_of_alpha, sv_cursor_segments);
...
void
ImGui::ColorTooltip(const char *text, const float *col, ImGuiColorEditFlags flags)
{
    ImGuiContext &g = *GImGui;
...
bool
ImGui::TreeNodeExV(const char *str_id, ImGuiTreeNodeFlags flags, const char *fmt, va_list args)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
    {
...
bool
ImGui::TreeNodeExV(const void *ptr_id, ImGuiTreeNodeFlags flags, const char *fmt, va_list args)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
    {
...
bool
ImGui::TreeNodeGetOpen(ImGuiID storage_id)
{
    ImGuiContext &g       = *GImGui;
...
void
ImGui::TreeNodeSetOpen(ImGuiID storage_id, bool open)
{
    ImGuiContext &g       = *GImGui;
    ImGuiStorage *storage = g.CurrentWindow->DC.StateStorage;
...
bool
ImGui::TreeNodeUpdateNextOpen(ImGuiID storage_id, ImGuiTreeNodeFlags flags)
{
...
// easy to increase.
static void
TreeNodeStoreStackData(ImGuiTreeNodeFlags flags)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = g.CurrentWindow;
...
bool
ImGui::CollapsingHeader(const char *label, bool *p_visible, ImGuiTreeNodeFlags flags)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
    {
...
// to next.
int
ImGui::TypingSelectFindNextSingleCharMatch(ImGuiTypingSelectRequest *req,
                                           int                       items_count,
                                           const char *(*get_item_name_func)(void *, int),
                                           void *user_data,
                                           int   nav_item_idx)
...
static void
BoxSelectPreStartDrag(ImGuiID id, ImGuiSelectionUserData clicked_item)
{
    ImGuiContext        &g    = *GImGui;
...
static void
BoxSelectActivateDrag(ImGuiBoxSelectState *bs, ImGuiWindow *window)
{
    ImGuiContext &g = *GImGui;
...
static void
BoxSelectScrollWithMouseDrag(ImGuiBoxSelectState *bs, ImGuiWindow *window, const ImRect &inner_r)
{
    ImGuiContext &g = *GImGui;
...
bool
ImGui::BeginBoxSelect(const ImRect         &scope_rect,
                      ImGuiWindow          *window,
...
void
ImGui::EndBoxSelect(const ImRect &scope_rect, ImGuiMultiSelectFlags ms_flags)
{
    ImGuiContext        &g      = *GImGui;
    ImGuiWindow         *window = g.CurrentWindow;
    ImGuiBoxSelectState *bs     = &g.BoxSelectState;
...
static void
DebugLogMultiSelectRequests(const char *function, const ImGuiMultiSelectIO *io)
{
    ImGuiContext &g = *GImGui;
    IM_UNUSED(function);
...
static ImRect
CalcScopeRect(ImGuiMultiSelectTempData *ms, ImGuiWindow *window)
{
    ImGuiContext &g = *GImGui;
...
ImGuiMultiSelectIO *
ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = g.CurrentWindow;
...
void
ImGui::MultiSelectItemHeader(ImGuiID id, bool *p_selected, ImGuiButtonFlags *p_button_flags)
{
    ImGuiContext             &g  = *GImGui;
    ImGuiMultiSelectTempData *ms = g.CurrentMultiSelect;
...
// imgui_test_suite.
void
ImGui::MultiSelectItemFooter(ImGuiID id, bool *p_selected, bool *p_pressed)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = g.CurrentWindow;
    ...
    if (pressed && (!enter_pressed || !selected))
    {
        ...
        MultiSelectAddSetRange(
            ms, range_selected, range_direction, storage->RangeSrcItem, item_data);
    }
...
void
ImGui::MultiSelectAddSetAll(ImGuiMultiSelectTempData *ms, bool selected)
{
...
void
ImGui::MultiSelectAddSetRange(ImGuiMultiSelectTempData *ms,
                              bool                      selected,
                              int                       range_dir,
...
void
ImGui::DebugNodeMultiSelectState(ImGuiMultiSelectState *storage)
{
#ifndef IMGUI_DISABLE_DEBUG_TOOLS
...
void
ImGuiSelectionBasicStorage::Clear()
{
    Size            = 0;
    _SelectionOrder = 1; // Always >0
...
void
ImGuiSelectionBasicStorage::Swap(ImGuiSelectionBasicStorage &r)
{
    ImSwap(Size, r.Size);
...
bool
ImGuiSelectionBasicStorage::Contains(ImGuiID id) const
{
...
bool
ImGuiSelectionBasicStorage::GetNextSelectedItem(void **opaque_it, ImGuiID *out_id)
{
...
void
ImGuiSelectionBasicStorage::SetItemSelected(ImGuiID id, bool selected)
{
...
static void
ImGuiSelectionBasicStorage_BatchSetItemSelected(ImGuiSelectionBasicStorage *selection,
                                                ImGuiID                     id,
                                                bool                        selected,
...
//   }
void
ImGuiSelectionBasicStorage::ApplyRequests(ImGuiMultiSelectIO *ms_io)
{
...
void
ImGuiSelectionExternalStorage::ApplyRequests(ImGuiMultiSelectIO *ms_io)
{
...
float
ImGuiMenuColumns::DeclColumns(float w_icon, float w_label, float w_shortcut, float w_mark)
{
        ...
        && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
            ...
            && (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded) == 0)
        {
            ...
            NavMoveRequestForward(
                g.NavMoveDir, g.NavMoveClipDir, g.NavMoveFlags, g.NavMoveScrollFlags); // Repeat
        }
    }
    ...
    else
    {
        ...
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
        {
            ...
            if (selected)
            {
                RenderCheckMark(window->DrawList,
                                pos
                                    + ImVec2(offsets->OffsetMark + stretch_w + g.FontSize * 0.40f,
...
bool
ImGui::MenuItem(const char *label, const char *shortcut, bool selected, bool enabled)
{
...
bool
ImGui::BeginTabBarEx(ImGuiTabBar *tab_bar, const ImRect &tab_bar_bb, ImGuiTabBarFlags flags)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = g.CurrentWindow;
    if (window->SkipItems)
...
static ImU32
ImGui::TabBarCalcTabID(ImGuiTabBar *tab_bar, const char *label, ImGuiWindow *docked_window)
{
...
static void
ImGui::TabBarScrollToTab(ImGuiTabBar *tab_bar, ImGuiID tab_id, ImGuiTabBarSection *sections)
{
...
void
ImGui::TabBarQueueFocus(ImGuiTabBar *tab_bar, const char *tab_name)
{
...
void
ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar *tab_bar, ImGuiTabItem *src_tab, ImVec2 mouse_pos)
{
    ImGuiContext &g = *GImGui;
    IM_ASSERT(tab_bar->ReorderRequestTabId == 0);
...
ImGui::TabItemButton(const char *label, ImGuiTabItemFlags flags)
{
    ...
    return TabItemEx(
        tab_bar, label, NULL, flags | ImGuiTabItemFlags_Button | ImGuiTabItemFlags_NoReorder, NULL);
}

void
ImGui::TabItemSpacing(const char *str_id, ImGuiTabItemFlags flags, float width)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow  *window = g.CurrentWindow;
    if (window->SkipItems)
...
ImVec2
ImGui::TabItemCalcSize(const char *label, bool has_close_button_or_unsaved_marker)
{
    ImGuiContext &g          = *GImGui;
    ImVec2        label_size = CalcTextSize(label, NULL, true);
