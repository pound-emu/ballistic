...
//  (ImGuiBackendFlags_RendererHasVtxOffset). [X] Renderer: Expose selected render state for draw
//  callbacks to use. Access in '(ImGui_ImplXXXX_RenderState*)GetPlatformIO().Renderer_RenderState'.
...
//  2025-02-26: Recreate image bind groups during render. (#8426, #8046, #7765, #8027) + Update for
//  latest webgpu-native changes. 2024-10-14: Update Dawn support for change of string usages.
//  (#8082, #8083) 2024-10-07: Expose selected render state in ImGui_ImplWGPU_RenderState, which you
//  can access in 'void* platform_io.Renderer_RenderState' during draw callbacks. 2024-10-07:
//  Changed default texture sampler to Clamp instead of Repeat/Wrap. 2024-09-16: Added support for
//  optional IMGUI_IMPL_WEBGPU_BACKEND_DAWN / IMGUI_IMPL_WEBGPU_BACKEND_WGPU define to handle
//  ever-changing native implementations. (#7977) 2024-01-22: Added configurable
//  PipelineMultisampleState struct. (#7240) 2024-01-22: (Breaking) ImGui_ImplWGPU_Init() now takes
//  a ImGui_ImplWGPU_InitInfo structure instead of variety of parameters, allowing for easier
//  further changes. 2024-01-22: Fixed pipeline layout leak. (#7245) 2024-01-17: Explicitly fill all
//  of WGPUDepthStencilState since standard removed defaults. 2023-07-13: Use
//  WGPUShaderModuleWGSLDescriptor's code instead of source. use WGPUMipmapFilterMode_Linear instead
//  of WGPUFilterMode_Linear. (#6602) 2023-04-11: Align buffer sizes. Use WGSL shaders instead of
//  precompiled SPIR-V. 2023-04-11: Reorganized backend to pull data from a single structure to
//  facilitate usage with multiple-contexts (all g_XXXX access changed to bd->XXXX). 2023-01-25:
//  Revert automatic pipeline layout generation (see https://github.com/gpuweb/gpuweb/issues/2470)
//  2022-11-24: Fixed validation error with default depth buffer settings.
//  2022-11-10: Fixed rendering when a depth buffer is enabled. Added 'WGPUTextureFormat
//  depth_format' parameter to ImGui_ImplWGPU_Init(). 2022-10-11: Using 'nullptr' instead of 'NULL'
//  as per our switch to C++11. 2021-11-29: Passing explicit buffer sizes to
...
static void
SafeRelease(WGPUPipelineLayout &res)
{
    if (res)
    {
...
ImGui_ImplWGPU_CreateImageBindGroup(WGPUBindGroupLayout layout, WGPUTextureView texture)
{
    ImGui_ImplWGPU_Data *bd                 = ImGui_ImplWGPU_GetBackendData();
...
ImGui_ImplWGPU_RenderDrawData(ImDrawData *draw_data, WGPURenderPassEncoder pass_encoder)
{
    ...
    wgpuQueueWriteBuffer(
        bd->defaultQueue, fr->VertexBuffer, 0, fr->VertexBufferHost, vb_write_size);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Clamp to viewport as wgpuRenderPassEncoderSetScissorRect() won't accept values
                // that are off bounds
                if (clip_min.x < 0.0f)
                {
                    clip_min.x = 0.0f;
...
bool
ImGui_ImplWGPU_Init(ImGui_ImplWGPU_InitInfo *init_info)
{
    ImGuiIO &io = ImGui::GetIO();
    IMGUI_CHECKVERSION();
