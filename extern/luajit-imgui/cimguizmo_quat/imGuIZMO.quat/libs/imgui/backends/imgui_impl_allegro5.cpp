...
//  [X] Renderer: User texture binding. Use 'ALLEGRO_BITMAP*' as ImTextureID. Read the FAQ about
//  ImTextureID! [X] Platform: Keyboard support. Since 1.87 we are using the io.AddKeyEvent()
//  function. Pass ImGuiKey values to all key functions e.g. ImGui::IsKeyPressed(ImGuiKey_Space).
//  [Legacy ALLEGRO_KEY_* values are obsolete since 1.87 and not supported since 1.91.5] [X]
//  Platform: Clipboard support (from Allegro 5.1.12). [X] Platform: Mouse cursor shape and
...
//  2025-01-06: Avoid calling al_set_mouse_cursor() repeatedly since it appears to leak on on X11
//  (#8256). 2024-08-22: moved some OS/backend related function pointers from ImGuiIO to
//  ImGuiPlatformIO:
...
ImGui_ImplAllegro5_RenderDrawData(ImDrawData *draw_data)
{
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        if (sizeof(ImDrawIdx) == 2)
        {
            ...
            // indices. Otherwise, we convert them from 16-bit to 32-bit at runtime here, which
            // works perfectly but is a little wasteful.
...
#else
        // Allegro's implementation of al_draw_indexed_prim() for DX9 was broken until 5.2.5.
        // Unindex buffers ourselves while converting vertex format.
...
static const char *
ImGui_ImplAllegro5_GetClipboardText(ImGuiContext *)
{
    ImGui_ImplAllegro5_Data *bd = ImGui_ImplAllegro5_GetBackendData();
...
static void
ImGui_ImplAllegro5_SetClipboardText(ImGuiContext *, const char *text)
{
    ImGui_ImplAllegro5_Data *bd = ImGui_ImplAllegro5_GetBackendData();
...
ImGui_ImplAllegro5_Init(ALLEGRO_DISPLAY *display)
{
    ...
    // Unfortunately Allegro doesn't support 32-bit packed colors so we have to convert them to 4
        ...
        case ALLEGRO_EVENT_KEY_UP:
            if (ev->keyboard.display == bd->Display)
            {
                ...
                io.SetKeyEventNativeData(
                    key, ev->keyboard.keycode, -1); // To support legacy indexing (<1.87 user code)
            }
            return true;
