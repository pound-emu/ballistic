...
//  SDL_GPUTextureSamplerBinding to ImTextureID. [X] Renderer: Large meshes support (64k+ vertices)
//  with 16-bit indices.
...
static void
ImGui_ImplSDLGPU3_SetupRenderState(ImDrawData                  *draw_data,
                                   SDL_GPUGraphicsPipeline     *pipeline,
                                   SDL_GPUCommandBuffer        *command_buffer,
                                   SDL_GPURenderPass           *render_pass,
                                   ImGui_ImplSDLGPU3_FrameData *fd,
                                   uint32_t                     fb_width,
                                   uint32_t                     fb_height)
{
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        SDL_BindGPUIndexBuffer(render_pass,
                               ...
                               sizeof(ImDrawIdx) == 2 ? SDL_GPU_INDEXELEMENTSIZE_16BIT
...
static void
CreateOrResizeBuffers(SDL_GPUBuffer         **buffer,
...
void
ImGui_ImplSDLGPU3_PrepareDrawData(ImDrawData *draw_data, SDL_GPUCommandBuffer *command_buffer)
{
    ...
    {
        CreateOrResizeBuffers(&fd->VertexBuffer,
                              &fd->VertexTransferBuffer,
                              &fd->VertexBufferSize,
                              vertex_size,
    ...
    {
        CreateOrResizeBuffers(&fd->IndexBuffer,
                              &fd->IndexTransferBuffer,
                              &fd->IndexBufferSize,
                              index_size,
                                 ...
                                 SDL_GPUGraphicsPipeline *pipeline)
{
    ...
    ImGui_ImplSDLGPU3_SetupRenderState(
        draw_data, pipeline, command_buffer, render_pass, fd, fb_width, fb_height);
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                SDL_BindGPUFragmentSamplers(
                    render_pass, 0, (SDL_GPUTextureSamplerBinding *)pcmd->GetTexID(), 1);
    ...
    // SDL_SetGPUViewport() and SDL_SetGPUScissor() yourself to explicitly set that state In theory
    // we should aim to backup/restore those values but I am not sure this is possible. We perform a
    // call to SDL_SetGPUScissor() to set back a full viewport which is likely to fix things for 99%
    // users but technically this is not perfect. (See github #4644)
...
// You probably never need to call this, as it is called by ImGui_ImplSDLGPU3_CreateFontsTexture()
// and ImGui_ImplSDLGPU3_Shutdown().
void
ImGui_ImplSDLGPU3_DestroyFontsTexture()
{
    ImGuiIO                    &io = ImGui::GetIO();
...
bool
ImGui_ImplSDLGPU3_Init(ImGui_ImplSDLGPU3_InitInfo *info)
{
    ImGuiIO &io = ImGui::GetIO();
    IMGUI_CHECKVERSION();
