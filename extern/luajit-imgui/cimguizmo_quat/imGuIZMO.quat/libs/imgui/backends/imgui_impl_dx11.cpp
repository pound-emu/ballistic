...
//  about ImTextureID! [X] Renderer: Large meshes support (64k+ vertices) even with 16-bit indices
...
//  2025-01-06: DirectX11: Expose VertexConstantBuffer in ImGui_ImplDX11_RenderState. Reset
//  projection matrix in ImDrawCallback_ResetRenderState handler. 2024-10-07: DirectX11: Changed
//  default texture sampler to Clamp instead of Repeat/Wrap. 2024-10-07: DirectX11: Expose selected
//  render state in ImGui_ImplDX11_RenderState, which you can access in 'void*
//  platform_io.Renderer_RenderState' during draw callbacks. 2022-10-11: Using 'nullptr' instead of
//  'NULL' as per our switch to C++11. 2021-06-29: Reorganized backend to pull data from a single
//  structure to facilitate usage with multiple-contexts (all g_XXXX access changed to bd->XXXX).
//  2021-05-19: DirectX11: Replaced direct access to ImDrawCmd::TextureId with a call to
//  ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18: DirectX11: Change blending
//  equation to preserve alpha in output buffer. 2019-08-01: DirectX11: Fixed code querying the
//  Geometry Shader state (would generally error with Debug layer enabled). 2019-07-21: DirectX11:
//  Backup, clear and restore Geometry Shader is any is bound when calling
...
// Functions
static void
ImGui_ImplDX11_SetupRenderState(ImDrawData *draw_data, ID3D11DeviceContext *device_ctx)
{
...
ImGui_ImplDX11_DestroyFontsTexture()
{
    ...
    if (bd->pFontTextureView)
    {
        ...
        ImGui::GetIO().Fonts->SetTexID(
            0); // We copied data->pFontTextureView to io.Fonts->TexID so let's clear that as well.
    }
}
...
ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(D3DCompile(vertexShader,
                              ...
                              nullptr)))
        {
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in
