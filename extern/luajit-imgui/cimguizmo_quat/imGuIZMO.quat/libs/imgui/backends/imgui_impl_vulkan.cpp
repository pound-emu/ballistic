...
//  [!] Renderer: User texture binding. Use 'VkDescriptorSet' as ImTextureID. Call
//  ImGui_ImplVulkan_AddTexture() to register one. Read the FAQ about ImTextureID! See
...
//  2025-02-13: Vulkan: Added ApiVersion field in ImGui_ImplVulkan_InitInfo. Default to header
//  version if unspecified. Dynamic rendering path loads "vkCmdBeginRendering/vkCmdEndRendering"
//  (without -KHR suffix) on API 1.3. (#8326) 2025-01-09: Vulkan: Added
//  IMGUI_IMPL_VULKAN_MINIMUM_IMAGE_SAMPLER_POOL_SIZE to clarify how many image sampler descriptors
//  are expected to be available in descriptor pool. (#6642) 2025-01-06: Vulkan: Added more
//  ImGui_ImplVulkanH_XXXX helper functions to simplify our examples. 2024-12-11: Vulkan: Fixed
...
AlignBufferSize(VkDeviceSize size, VkDeviceSize alignment)
{
                                ...
                                VkPipeline      pipeline)
{
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        for (int n = 0; n < draw_data->CmdListsCount; n++)
        {
            ...
            memcpy(
                vtx_dst, draw_list->VtxBuffer.Data, draw_list->VtxBuffer.Size * sizeof(ImDrawVert));
            memcpy(
                idx_dst, draw_list->IdxBuffer.Data, draw_list->IdxBuffer.Size * sizeof(ImDrawIdx));
...
ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    // Copy to Image:
    {
        ...
        vkCmdCopyBufferToImage(bd->TexCommandBuffer,
                               upload_buffer,
                               backend_tex->Image,
    ...
    void *user_data)
{
    ...
    ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR = reinterpret_cast<PFN_vkCmdEndRenderingKHR>(
        loader_func(api_version < VK_API_VERSION_1_3 ? "vkCmdEndRenderingKHR" : "vkCmdEndRendering",
                    user_data));
}
#endif
...
static uint32_t
ImGui_ImplVulkan_GetDefaultApiVersion()
{
...
bool
ImGui_ImplVulkan_LoadFunctions(uint32_t api_version,
...
void
ImGui_ImplVulkan_DestroyFrameRenderBuffers(VkDevice                             device,
                                           ImGui_ImplVulkan_FrameRenderBuffers *buffers,
                                           const VkAllocationCallbacks         *allocator)
{
...
void
ImGui_ImplVulkan_DestroyWindowRenderBuffers(VkDevice                              device,
                                            ImGui_ImplVulkan_WindowRenderBuffers *buffers,
                                            const VkAllocationCallbacks          *allocator)
{
...
void
ImGui_ImplVulkanH_CreateWindowCommandBuffers(VkPhysicalDevice             physical_device,
                                             VkDevice                     device,
                                             ImGui_ImplVulkanH_Window    *wd,
                                             uint32_t                     queue_family,
                                             const VkAllocationCallbacks *allocator)
