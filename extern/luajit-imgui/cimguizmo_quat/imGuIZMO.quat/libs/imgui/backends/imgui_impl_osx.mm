...
//  2025-03-21: Fill gamepad inputs and set ImGuiBackendFlags_HasGamepad regardless of
//  ImGuiConfigFlags_NavEnableGamepad being set. 2025-01-20: Removed notification observer when
//  shutting down. (#8331) 2024-08-22: moved some OS/backend related function pointers from ImGuiIO
//  to ImGuiPlatformIO:
//               - io.GetClipboardTextFn    -> platform_io.Platform_GetClipboardTextFn
//               - io.SetClipboardTextFn    -> platform_io.Platform_SetClipboardTextFn
//               - io.PlatformSetImeDataFn  -> platform_io.Platform_SetImeDataFn
//  2024-07-02: Update for io.SetPlatformImeDataFn() -> io.PlatformSetImeDataFn() renaming in main
//  library. 2023-10-05: Inputs: Added support for extra ImGuiKey values: F13 to F20 function keys.
//  Stopped mapping F13 into PrintScreen. 2023-04-09: Inputs: Added support for
//  io.AddMouseSourceEvent() to discriminate ImGuiMouseSource_Mouse/ImGuiMouseSource_Pen.
//  2023-02-01: Fixed scroll wheel scaling for devices emitting events with
//  hasPreciseScrollingDeltas==false (e.g. non-Apple mice). 2022-11-02: Fixed mouse coordinates
//  before clicking the host window. 2022-10-06: Fixed mouse inputs on flipped views. 2022-09-26:
//  Inputs: Renamed ImGuiKey_ModXXX introduced in 1.87 to ImGuiMod_XXX (old names still supported).
//  2022-05-03: Inputs: Removed ImGui_ImplOSX_HandleEvent() from backend API in favor of backend
//  automatically handling event capture. 2022-04-27: Misc: Store backend data in a per-context
//  struct, allowing to use this backend with multiple contexts. 2022-03-22: Inputs: Monitor NSKeyUp
//  events to catch missing keyUp for key when user press Cmd + key 2022-02-07: Inputs: Forward
//  keyDown/keyUp events to OS when unused by dear imgui. 2022-01-31: Fixed building with old Xcode
//  versions that are missing gamepad features. 2022-01-26: Inputs: replaced short-lived
...
//  2022-01-12: Inputs: Added basic Platform IME support, hooking the io.SetPlatformImeDataFn()
//  function. 2022-01-10: Inputs: calling new io.AddKeyEvent(), io.AddKeyModsEvent() +
//  io.SetKeyEventNativeData() API (1.87+). Support for full ImGuiKey range. 2021-12-13: *BREAKING
//  CHANGE* Add NSView parameter to ImGui_ImplOSX_Init(). Generally fix keyboard support. Using
//  kVK_* codes for keyboard keys. 2021-12-13: Add game controller support. 2021-09-21: Use
//  mach_absolute_time as CFAbsoluteTimeGetCurrent can jump backwards. 2021-08-17: Calling
//  io.AddFocusEvent() on
...
{
    IM_DELETE(ImGui_ImplOSX_GetBackendData());
}
...
ImGuiKey
ImGui_ImplOSX_KeyCodeToImGuiKey(int key_code)
{
    switch (key_code)
    {
        case kVK_ANSI_A:
...
ImGui_ImplOSX_Init(NSView *view)
{
    ...
    platform_io.Platform_SetImeDataFn
        = [](ImGuiContext *, ImGuiViewport *viewport, ImGuiPlatformImeData *data) -> void {
...
ImGui_ImplOSX_UpdateMouseCursor()
{
    ...
    {
        NSCursor *desired
            = bd->MouseCursors[imgui_cursor] ?: bd->MouseCursors[ImGuiMouseCursor_Arrow];
...
static void
ImGui_ImplOSX_UpdateImePosWithView(NSView *view)
{
    ImGui_ImplOSX_Data *bd = ImGui_ImplOSX_GetBackendData();
    ImGuiIO            &io = ImGui::GetIO();
    if (io.WantTextInput)
...
void
ImGui_ImplOSX_NewFrame(NSView *view)
{
    ImGui_ImplOSX_Data *bd = ImGui_ImplOSX_GetBackendData();
    IM_ASSERT(bd != nullptr
              && "Context or backend not initialized! Did you call ImGui_ImplOSX_Init()?");
    ImGuiIO &io = ImGui::GetIO();
...
GetMouseSource(NSEvent *event)
{
    switch (event.subtype)
    {
        case NSEventSubtypeTabletPoint:
            return ImGuiMouseSource_Pen;
        // macOS considers input from relative touch devices (like the trackpad or Apple Magic
        // Mouse) to be touch input. This doesn't really make sense for Dear ImGui, which expects
        // absolute touch devices only. There does not seem to be a simple way to disambiguate
        // things here so we consider NSEventSubtypeTouch events to always come from mice. See
...
static bool
ImGui_ImplOSX_HandleEvent(NSEvent *event, NSView *view)
{
    ImGuiIO &io = ImGui::GetIO();
    ...
    if (event.type == NSEventTypeScrollWheel)
    {
        ...
        //
        // 2. A scroll wheel NSEvent, with a phase == NSEventPhaseCancelled, when the user releases
        // the two-finger tap. It is this event that sometimes contains large values for
        // scrollingDeltaX and scrollingDeltaY. When these are added to the current x and y
        // positions of the scrolling view, it appears to jump up or down. It can be observed in
        // Preview, various JetBrains IDEs and here.
...
static void
ImGui_ImplOSX_AddTrackingArea(NSView *_Nonnull view)
{
    ...
    // event stream. To match the behavior of other backends, we pass every event down to the OS.
    ImGui_ImplOSX_Data *bd = ImGui_ImplOSX_GetBackendData();
    if (bd->Monitor)
    {
        return;
