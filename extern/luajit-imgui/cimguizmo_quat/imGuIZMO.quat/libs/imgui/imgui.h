...
// - You can make this a structure with various constructors if you need. You will have to implement
// ==/!= operators.
// - (note: before v1.91.4 (2024/10/08) the default type for ImTextureID was void*. Use intermediary
// intptr_t cast and read FAQ if you have casting warnings)
#ifndef ImTextureID
typedef ImU64 ImTextureID; // Default: store a pointer or an integer fitting in a pointer (most
...
IMGUI_API void ShowAboutWindow(bool *p_open
                               = NULL); // create About window. display Dear ImGui version, credits
...
IMGUI_API bool IsWindowHovered(
    ...
    = 0); // is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See
          ...
          // mouse should be dispatched to Dear ImGui or to your underlying app, you should not use
          // this function! Use the 'io.WantCaptureMouse' boolean for that! Refer to FAQ entry "How
          // can I tell whether to dispatch mouse/keyboard to Dear ImGui or my application?" for
          // details.
...
IMGUI_API float GetWindowHeight(); // get current window height (IT IS UNLIKELY YOU EVER NEED TO USE
                                   // THIS). Shortcut for GetWindowSize().y.
...
IMGUI_API void SetWindowPos(
    const ImVec2 &pos,
    ImGuiCond     cond
    = 0); // (not recommended) set current window position - call within Begin()/End(). prefer using
...
IMGUI_API void SetScrollHereX(
    ...
    = 0.5f); // adjust scrolling amount to make current cursor position visible. center_x_ratio=0.0:
...
IMGUI_API void SetScrollHereY(
    ...
    = 0.5f); // adjust scrolling amount to make current cursor position visible. center_y_ratio=0.0:
...
IMGUI_API ImU32 GetColorU32(ImU32 col,
                            float alpha_mul
                            = 1.0f); // retrieve given color with style alpha applied, packed as a
...
IMGUI_API ImVec2 GetCursorScreenPos(); // cursor position, absolute coordinates. THIS IS YOUR BEST
                                       // FRIEND (prefer using this rather than GetCursorPos(), also
                                       // more useful to work with ImDrawList API).
...
IMGUI_API ImVec2 GetCursorPos(); // [window-local] cursor position in window-local coordinates. This
...
IMGUI_API bool Combo(const char *label,
                     ...
                     = -1); // Separate items with \0 within a string, end item-list with \0\0. e.g.
                            // "One\0Two\0Three\0"
IMGUI_API bool Combo(const char *label,
                     int        *current_item,
...
IMGUI_API void OpenPopupOnItemClick(
    ...
    = 1); // helper to open popup when clicked on last item. Default to
          // ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_
...
IMGUI_API bool BeginPopupContextItem(
    const char     *str_id = NULL,
    ImGuiPopupFlags popup_flags
    = 1); // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to
...
IMGUI_API bool BeginDragDropSource(
    ...
    = 0); // call after submitting an item which may be dragged. when this return true, you can call
...
IMGUI_API void SetNextFrameWantCaptureMouse(
    bool want_capture_mouse); // Override io.WantCaptureMouse flag next frame (said flag is left for
                              // your application to handle, typical when true it instructs your app
...
enum ImGuiInputTextFlags_
{
    ...
    = 1 << 22, // Callback on buffer capacity changes request (beyond 'buf_size' parameter value),
               ...
               // string types which hold a cache of their Size). You will be provided a new BufSize
               // in the callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h for an example
               // of using this)
    ImGuiInputTextFlags_CallbackEdit
    = 1 << 23, // Callback on any edit. Note that InputText() already returns true on edit + you can
               // always use IsItemEdited(). The callback is useful to manipulate the underlying
...
enum ImGuiTreeNodeFlags_
{
    ...
    = 1 << 10, // Use FramePadding (even for an unframed text node) to vertically align text
               // baseline to regular widget height. Equivalent to calling AlignTextToFramePadding()
               // before the node.
    ImGuiTreeNodeFlags_SpanAvailWidth
    = 1 << 11, // Extend hit box to the right-most edge, even if not framed. This is not the default
               // in order to allow adding other items on the same line without using AllowOverlap
               // mode.
    ...
    = 1 << 17, // (WIP) Nav: left direction may move to this TreeNode() from any of its child (items
...
enum ImGuiPopupFlags_
{
    ...
    = 0, // For BeginPopupContext*(): open on Left Mouse release. Guaranteed to always be == 0 (same
...
enum ImGuiDragDropFlags_
{
    ...
    = 1 << 0, // Disable preview tooltip. By default, a successful call to BeginDragDropSource opens
              // a tooltip so you can display a preview or description of the source contents. This
    ...
    = 1 << 3, // Allow items such as Text(), Image() that have no unique identifier to be used as
              ...
              // position. This is extremely unusual within the dear imgui ecosystem and so we made
              // it explicit.
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ImGuiKey_COUNT
    = ImGuiKey_NamedKey_END, // Obsoleted in 1.91.5 because it was extremely misleading (since named
...
enum ImGuiInputFlags_
{
    ...
    = 1 << 16, // Option: global route: will not be applied if underlying background/void is focused
...
enum ImGuiConfigFlags_
{
    ...
    = 1 << 5, // Instruct backend to not alter mouse cursor shape and visibility. Use if the backend
              // cursor changes are interfering with yours and you don't want to use
...
enum ImGuiSliderFlags_
{
    ...
    = 1 << 11, // Disable keyboard modifiers altering tweak speed. Useful if you want to alter tweak
...
enum ImGuiTableFlags_
{
    ...
    = 3 << 13, // Columns default to _WidthStretch with default weights proportional to each columns
               // contents widths.
...
enum ImGuiTableColumnFlags_
{
    ...
    = 1 << 18, // TableHeadersRow() will submit an angled header row for this column. Note this will
               // add an extra row.
...
enum ImGuiTableRowFlags_
{
    ...
    = 1 << 0, // Identify header row (set default background color + width of its contents accounted
...
struct ImGuiStyle
{
    ...
    ImVec2 DisplayWindowPadding; // Apply to regular windows: amount which we enforce to keep
...
struct ImGuiIO
{
    ...
    ImGuiKeyData KeysData[ImGuiKey_NamedKey_COUNT]; // Key state for all known keys. Use IsKeyXXX()
...
enum ImGuiMultiSelectFlags_
{
    ...
    = 1 << 2, // Disable Shift+selection mouse/keyboard support (useful for unordered 2D selection).
              // With BoxSelect is also ensure contiguous SetRange requests are not combined into
              // one. This allows not handling interpolation in SetRange requests.
    ImGuiMultiSelectFlags_NoAutoSelect
    = 1 << 3, // Disable selecting items when navigating (useful for e.g. supporting range-select in
              // a list of checkboxes).
    ImGuiMultiSelectFlags_NoAutoClear
    = 1 << 4, // Disable clearing selection when navigating or selecting another one (generally used
              // with ImGuiMultiSelectFlags_NoAutoSelect. useful for e.g. supporting range-select in
              // a list of checkboxes).
    ...
    = 1 << 16, // [Temporary] Enable navigation wrapping on X axis. Provided as a convenience
               // because we don't have a design for the general Nav API for this yet. When the more
               // general feature be public we may obsolete this flag in favor of new one.
};
...
struct ImDrawList
{
    ...
    // Obsolete names
    // inline  void  AddEllipse(const ImVec2& center, float radius_x, float radius_y, ImU32 col,
    // float rot = 0.0f, int num_segments = 0, float thickness = 1.0f) { AddEllipse(center,
    // ImVec2(radius_x, radius_y), col, rot, num_segments, thickness); } // OBSOLETED in 1.90.5 (Mar
    // 2024) inline  void  AddEllipseFilled(const ImVec2& center, float radius_x, float radius_y,
    // ImU32 col, float rot = 0.0f, int num_segments = 0) { AddEllipseFilled(center,
    // ImVec2(radius_x, radius_y), col, rot, num_segments); }                        // OBSOLETED
    // in 1.90.5 (Mar 2024) inline  void  PathEllipticalArcTo(const ImVec2& center, float radius_x,
    // float radius_y, float rot, float a_min, float a_max, int num_segments = 0) {
    // PathEllipticalArcTo(center, ImVec2(radius_x, radius_y), rot, a_min, a_max, num_segments); }
    // // OBSOLETED in 1.90.5 (Mar 2024) inline  void  AddBezierCurve(const ImVec2& p1, const
    // ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness, int num_segments
    // = 0) { AddBezierCubic(p1, p2, p3, p4, col, thickness, num_segments); } // OBSOLETED in 1.80
    // (Jan 2021) inline  void  PathBezierCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2&
    // p4, int num_segments = 0) { PathBezierCubicCurveTo(p2, p3, p4, num_segments); } // OBSOLETED
    // in 1.80 (Jan 2021)
...
struct ImFontConfig
{
    ...
    int OversampleH; // 0 (2)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto
                     ...
                     // details.
    int OversampleV; // 0 (1)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto
    ...
    // ImVec2        GlyphExtraSpacing;      // 0, 0     // (REMOVED IN 1.91.9: use
    // GlyphExtraAdvanceX)
    ...
    float GlyphExtraAdvanceX;      // 0        // Extra spacing (in pixels) between glyphs. Please
                                   // contact us if you are using this.
    ...
    float RasterizerDensity; // 1.0f     // DPI scale for rasterization, not altering other font
                             ...
                             // for a zooming display, or handle Retina screen. IMPORTANT: If you
                             // change this it is expected that you increase/decrease font scale
...
struct ImFontGlyph
{
    unsigned int Colored
        : 1; // Flag to indicate glyph is colored and should generally ignore tinting (make it
...
struct ImFontAtlasCustomRect
{
    ...
    unsigned int   GlyphColored
        : 1; // Input  // For custom font glyphs only: glyph is colored, removed tinting.
...
struct ImFontAtlas
{
    ...
    IMGUI_API void ClearFonts(); // Clear input+output font data (same as ClearInputData() + glyphs
    ...
    // backends decide of preferred texture format.
...
struct ImFont
{
    ...
    ImFontConfig *Sources;        // 4-8   // in  // Pointer within ContainerAtlas->Sources[], to
                                  // SourcesCount instances
    short SourcesCount; // 2     // in  // Number of ImFontConfig involved in creating this font.
                        // Usually 1, or >1 when merging multiple font sources into one ImFont.
    short   EllipsisCharCount; // 1     // out // 1 or 3
    ImWchar EllipsisChar;      // 2-4   // out // Character used for ellipsis rendering ('...').
    ImWchar FallbackChar;     // 2-4   // out // Character used if a glyph isn't found (U+FFFD, '?')
    float   EllipsisWidth;    // 4     // out // Total ellipsis Width
    float   EllipsisCharStep; // 4     // out // Step between characters when EllipsisCount > 0
    float Scale; // 4     // in  // Base font scale (1.0f), multiplied by the per-window font scale
                 // which you can adjust with SetWindowFontScale()
    ...
    int MetricsTotalSurface; // 4     // out // Total surface in pixels to get an idea of the font
                             // rasterization/texture cost (not exact, we approximate the cost of
...
static inline bool
BeginChildFrame(ImGuiID id, const ImVec2 &size, ImGuiWindowFlags window_flags = 0)
{
...
// static inline bool BeginChild(const char* str_id, const ImVec2& size_arg, bool borders,
...
// ImGuiChildFlags_Borders static inline bool BeginChild(ImGuiID id, const ImVec2& size_arg, bool
