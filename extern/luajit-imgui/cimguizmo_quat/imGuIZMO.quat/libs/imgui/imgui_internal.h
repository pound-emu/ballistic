...
static inline ImVec2
ImClamp(const ImVec2 &v, const ImVec2 &mn, const ImVec2 &mx)
{
...
static inline float
ImLinearRemapClamp(float s0, float s1, float d0, float d1, float x)
{
...
{
    ImVector<ImDrawList *> *Layers[2]; // Pointers to global layers for: regular, tooltip.
...
struct IMGUI_API ImGuiTable
{
    ...
    bool IsContextPopupOpen; // Set when default context menu is open (also see: ContextPopupColumn,
                             // InstanceInteracted).
    bool DisableDefaultContextMenu; // Disable default context menu contents. You may submit your
                                    // own using TableBeginContextMenuPopup()/EndPopup()
...
IMGUI_API void FindHoveredWindowEx(const ImVec2 &pos,
...
IMGUI_API void RenderFrame(
    ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool borders = true, float rounding = 0.0f);
...
IMGUI_API void ImFontAtlasBuildSetupFont(
    ImFontAtlas *atlas, ImFont *font, ImFontConfig *src, float ascent, float descent);
...
IMGUI_API void ImFontAtlasBuildGetOversampleFactors(const ImFontConfig *src,
