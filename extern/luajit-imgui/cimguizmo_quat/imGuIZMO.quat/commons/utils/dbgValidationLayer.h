...
public:
    ...
    {
        auto createDebugUtilsMessenger = [](vk::Instance                               &instance,
        ...
        constexpr auto severityFlags
            = severityFlag::eVerbose | severityFlag::eWarning | severityFlag::eError
              | severityFlag::eInfo; // severityFlag::eInfo ==> add for more Infos
        constexpr auto typeFlags
            = typeFlag::eGeneral | typeFlag::eValidation | typeFlag::ePerformance;

        const vk::DebugUtilsMessengerCreateInfoEXT createInfo
            = { {}, severityFlags, typeFlags, debugCallback };
        if (createDebugUtilsMessenger(instance, &createInfo, nullptr, &debugMessenger)
            != VK_SUCCESS)
        {
    ...
    /////////////////////////////////////////////
    bool checkValLayer() const
    { // check validation layer support
        uint32_t layerCount;
        vk::detail::resultCheck(vk::enumerateInstanceLayerProperties(&layerCount, nullptr),
...
#define CHECK_VALIDATION_LAYER_SUPPORT()                                             \
    if (!debug.checkValLayer())                                                      \
    {                                                                                \
        throw std::runtime_error("validation layers requested, but not available!"); \
    }
#define PRINT_AVAILABLE_DEVICES(P)                                   \
    for (const auto &device : P)                                     \
    {                                                                \
