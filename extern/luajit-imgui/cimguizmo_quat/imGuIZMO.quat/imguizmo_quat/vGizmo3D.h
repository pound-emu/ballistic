...
public:
    virtualGizmoBaseClass()
        ...
        , tbSecControlButton(evRightButton)
        , tbSecControlModifiers(evNoModifier)
    ...
    {
        if ((button == tbControlButton) && pressed
            && (tbControlModifiers ? tbControlModifiers & mod : tbControlModifiers == mod))
        {
            tbActive = true;
            activateMouse(x, y);
        }
        if ((button == tbSecControlButton) && pressed
            && (tbSecControlModifiers ? tbSecControlModifiers & mod : tbSecControlModifiers == mod))
        {
            tbSecActive = true;
            activateMouse(x, y);
        }
    ...
    /// <b>Call in main render loop to implement a continue slow rotation for secondary
    /// trackball</b><br> <br> This rotation depends on speed of last mouse movements and maintains
    /// same spin <br> The speed can be adjusted from <b>setIdleRotSpeed(1.0)</b> It can be stopped
    /// by click on screen (without mouse movement)
    ///@code
    ///     while (!glfwWindowShouldClose(glfwWindow)) {
    ///         ...
    ...
    void idleSecond()
    {
        qtSecondRot = qtIdleSec * qtSecondRot;
    }
    ...
    void         updateGizmo()
    {
        if (delta.x == 0 && delta.y == 0)
        {
            ...
            if (tbActive)
            {
                qtIdle = tQuat(T(1), T(0), T(0), T(0));
            }
            if (tbSecActive)
            {
                qtIdleSec = tQuat(T(1), T(0), T(0), T(0));
            }
            return;
        }
        ...
        T angle = acos(
            AdotB > T(1)
                ...
                : (AdotB < -T(1)
                       ? -T(1)
                       : AdotB)); // clamp necessary!!! corss float is approximate to FLT_EPSILON

        auto flipRotation
            = [&](quat q) { return quat(q.w, rotOnX * q.x, rotOnY * q.y, rotOnZ * -q.z); };
    ...
    ///    track.setGizmoRotControl         (vg::evButton1  /* or vg::evLeftButton */, 0 /*
    ///    vg::evNoModifier */ );
    ///
    ///    // Rotations around specific axis: mouse button and key modifier
    ///    track.setGizmoRotXControl        (vg::evButton1  /* or vg::evLeftButton */,
    ///    vg::evShiftModifier); track.setGizmoRotYControl        (vg::evButton1  /* or
    ///    vg::evLeftButton */, vg::evControlModifier); track.setGizmoRotZControl (vg::evButton1  /*
    ///    or vg::evLeftButton */, vg::evAltModifier | vg::evSuperModifier);
    ///
    ///    // Set vGizmo3D control for secondary rotation
    ///    track.setGizmoSecondRotControl(vg::evButton2  /* or vg::evRightButton */, 0 /*
    ///    vg::evNoModifier */ );
    ///
    ///    // Pan and Dolly/Zoom: mouse button and key modifier
    ///    track.setDollyControl            (vg::evButton2 /* or vg::evRightButton */,
    ///    vg::evControlModifier); track.setPanControl              (vg::evButton2 /* or
    ///    vg::evRightButton */, vg::evShiftModifier);
    ///@endcode
    ///@note the example values are also DEFAULT values: you can omit to set they and to override
    /// only the associations that you want modify
    ...
    /// Set current rotation of vGizmo3D
    ///@param[in] q quat& : reference quaternion containing rotation to set
    void setSecondRot(const tQuat &q)
    {
        qtSecondRot = q;
    }
    ...
    /// flip X Rot
    ///@param[in] b bool
    void flipRotOnX(bool b = true)
    {
        rotOnX = b ? -T(1) : T(1);
    }
    /// flip Y Rot
    ///@param[in] b bool
    void flipRotOnY(bool b = true)
    {
        rotOnY = b ? -T(1) : T(1);
    }
    /// flip Z Rot
    ///@param[in] b bool
    void flipRotOnZ(bool b = true)
    {
        rotOnZ = b ? -T(1) : T(1);
    }
    /// flip Dolly mouse coord
    ///@param[in] b bool
    void setFlipDolly(bool b)
    {
        isFlipDolly = b;
    }
    /// flip Pan X mouse coord
    ///@param[in] b bool
    void setFlipPanX(bool b)
    {
        isFlipPanX = b;
    }
    /// flip Pan Y mouse coord
    ///@param[in] b bool
    void setFlipPanY(bool b)
    {
        isFlipPanY = b;
    }
...
protected:
    ...
    void setStepSecondRot(const tQuat &q)
    {
        qtStepSec = q;
    }
    ...
    T panFlipX(T x)
    {
        return isFlipPanX ? -x : x;
    }
    T panFlipY(T y)
    {
        return isFlipPanY ? -y : y;
    }
    T dollyFlip(T z)
    {
        return isFlipDolly ? -z : z;
    }

    tVec2 pos { 0 }, delta { 0 };

    // UI commands that this virtualGizmo responds to (defaults to left mouse button with no
    // modifier key)
...
public:
    ...
    void motion(T x, T y)
    {
        if (this->tbActive || this->tbSecActive)
        {
            VGIZMO_BASE_CLASS::motion(x, y);
        }
    }
...
public:
    ...
    virtualGizmo3DClass()
        ...
        , panControlButton(evRightButton)
        , dollyControlModifiers(evShiftModifier)
        , panControlModifiers(evControlModifier)
    {
    }
    ...
    void mouse(vgButtons button, vgModifiers mod, bool pressed, T x, T y)
    {
        ...
        if (button == dollyControlButton && pressed
        ...
        if (button == panControlButton && pressed
    ...
    void wheel(T x, T y, T z = T(0))
    {
        ...
        ;
        vecPanDolly.z += (y * dollyScale * wheelScale
                          * (povPanDollyFactor > T(0) ? povPanDollyFactor : T(1)));
    }
    ...
    void updateDolly()
    {
        vecPanDolly.z += this->dollyFlip(delta.y) * dollyScale * constPanDollyScale.z
                         * (povPanDollyFactor > T(0) ? povPanDollyFactor : T(1));
    }
    ...
    ///   */); t.setPanControl((vgButtons) GLFW_MOUSE_BUTTON_RIGHT, (vgModifiers) GLFW_MOD_ALT |
    ///   GLFW_MOD_SHIFT);
    ///@endcode
    ...
    {
        dollyScale = scale * constPanDollyScale.z;
    }
    ...
    {
        return dollyScale / constPanDollyScale.z;
    }
    ...
    {
        panScale = scale * constPanDollyScale.x;
    }
    ...
    {
        return panScale / constPanDollyScale.x;
    }
    ...
    //////////////////////////////////////////////////////////////////
    tVec3 getDollyPosition() const
    {
        return tVec3 { 0, 0, vecPanDolly.z };
    }

    //  Get Pan pos... use as Zoom factor
    //////////////////////////////////////////////////////////////////
    tVec3 getPanPosition() const
    {
        return tVec3 { vecPanDolly.x, vecPanDolly.y, 0 };
    }
...
private:
    ...
    T povPanDollyFactor
        = T(0); // internal use, maintain memory of current distance (pan/zoom speed by distance)
};
