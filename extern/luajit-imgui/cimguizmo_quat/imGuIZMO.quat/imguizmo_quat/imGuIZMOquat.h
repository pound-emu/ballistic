...
struct imguiGizmo
{
    ...
    ///                                                     // until not restoring all widgets are
    ///                                                     draw with this sizes
    ///        imguiGizmo::restoreAxesSize();               // restore at default axes length
    /// @endcode
    /// @note There is no a stack: a new call (w/o restoring) overwrite default values with previous
    /// ones
    ...
    static void setDollyWheelScale(float scale)
    {
        dollyWheelScale = scale;
    } // default 1.0, >1 more, <1 less
    static float getDollyWheelScale()
    {
        return dollyWheelScale;
    }
    ...
    /// flipX X coord
    ///@param[in] b bool
    static void flipRotOnX(bool b = true)
    {
        rotOnX = b;
    }
    /// flipY Y coord
    ///@param[in] b bool
    static void flipRotOnY(bool b = true)
    {
        rotOnY = b;
    }
    /// flipY Z coord
    ///@param[in] b bool
    static void flipRotOnZ(bool b = true)
    {
        rotOnZ = b;
    }
    ///@param[in] b bool
    static void setFlipDolly(bool b)
    {
        isFlipDolly = b;
    }
    /// flipZ mouse coord
    ///@param[in] b bool
    static void setFlipPanX(bool b)
    {
        isFlipPanX = b;
    }
    /// flipZ mouse coord
    ///@param[in] b bool
    static void setFlipPanY(bool b)
    {
        isFlipPanY = b;
    }
    ...
    /// flipX X coord
    ///@param[in] b bool
    static void reverseX(bool b = true)
    {
        reverseAxisX = b ? -1.f : 1.f;
    }
    static void reverseY(bool b = true)
    {
        reverseAxisY = b ? -1.f : 1.f;
    }
    static void reverseZ(bool b = true)
    {
        reverseAxisZ = b ? -1.f : 1.f;
    }

    /// get flip Rot X status
    /// @retval bool : current flip Rot X status
    static bool getReverseX()
    {
        return reverseAxisX < 0;
    }
    static bool getReverseY()
    {
        return reverseAxisY < 0;
    }
    static bool getReverseZ()
    {
        return reverseAxisZ < 0;
    }
    ...
    {
        drawMode       = uint32_t(mode & modeMask);
        axesOriginType = uint32_t(mode & axesModeMask);
        showFullAxes   = bool(modeFullAxes & mode);
    }

    void setDualMode(const uint32_t mode)
    {
    ...
    bool getTransforms(quat &q, const char *label, vec3 &vDir, float size)
    {
        ...
        if (ret)
        {
            dir = (q * vec3(1.0f, 0.0f, 0.0f)) * len; // return vector with original lenght
        }
#else
        q        = angleAxis(acosf(-dir.x / len), normalize(vec3(FLT_EPSILON, dir.z, -dir.y)));
        bool ret = drawFunc(label, size);
    ...
    {
        q = angleAxis(axis_angle.w,
                      normalize(checkTowards(vec3(axis_angle)))); // g.ConvertFromAxisAngle();

        bool ret = drawFunc(label, size);
        if (ret)
        {
            axis_angle = vec4(normalize(checkTowards(axis(q))), angle(normalize(q)));
        }

        return ret;
    }

    vec3 checkTowards(const vec3 &v)
    {
        return axesVecModifier * v;
    }
    vec4 checkTowards(const vec4 &v)
    {
        return { axesVecModifier.x * v.x, axesVecModifier.y * v.y, axesVecModifier.z * v.z, v.w };
    }
    quat checkTowards(const quat &q)
    {
        return { q.w, axesVecModifier.x * q.x, axesVecModifier.y * q.y, axesVecModifier.z * q.z };
    }
...
#define IMGUIZMO_DEF_SIZE \
    (ImGui::GetFrameHeightWithSpacing() * 4 - (ImGui::GetStyle().ItemSpacing.y * 2))
...
/// @param[in]     sz    <b> float       </b> - widget size: default size = FrameHeightWithSpacing*4
/// - ItemSpacing.y*2
/// @param[in]     flag  <b> uint32_t    </b> - masked flags to set modes (lower 16bit) and aspect
/// (higher 16bit)
/// @code
...
/// @endcode
IMGUI_API bool gizmo3D(const char *t,
                       quat       &q,
                       float       sz   = IMGUIZMO_DEF_SIZE,
                       uint32_t    flag = imguiGizmo::mode3Axes | imguiGizmo::cubeAtOrigin);
...
IMGUI_API bool gizmo3D(const char *t,
                       vec3       &v,
                       float       sz   = IMGUIZMO_DEF_SIZE,
                       uint32_t    flag = imguiGizmo::modeDirection);
...
IMGUI_API bool gizmo3D(const char *t,
                       ...
                       quat       &ql,
                       float       sz   = IMGUIZMO_DEF_SIZE,
                       uint32_t    flag = imguiGizmo::modeDual | imguiGizmo::cubeAtOrigin);
...
///
/// @param[in]     t     <b> const char *</b> - widget text: put "##" before the text to hide it
/// @param[in,out] vm    <b> vec3 &      </b> - Pan/Dolly position: (x, y) Pan position, (z)
/// Dolly/Zoom position
/// @param[in,out] q     <b> quat &      </b> - quaternion with axes rotation
/// @param[in]     sz    <b> float       </b> - widget size: default size = FrameHeightWithSpacing*4
/// - ItemSpacing.y*2
...
/// <b>vec4</b> p(xyz), w angle in radians for the spot<br>
///
/// Builds a ImGuIZMO_quat 3 axes + spot light from/with a "q" (quaternion) rotations of axes and a
/// vec4 (xyz axis, w angle in radians) for the spot + Pan & Dolly/Zoom capability
///
/// @param[in]     t     <b> const char *</b> - widget text: put "##" before the text to hide it
/// @param[in,out] vm    <b> vec3 &      </b> - Pan/Dolly position: (x, y) Pan position, (z)
/// Dolly/Zoom position
