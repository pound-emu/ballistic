...
vkApp::run()
{
    ...
    { // glfwPollEvents | SDL_PollEvent ... with exit/quit check
        ...
        ImGui::gizmo3D("##aaa",
                       rotation,
                       /* size */ 240); // if(ImGui::gizmo3D(...) == true) ---> widget has been
                                        // updated it returns "only" a rotation (net of
                                        // transformations) in base to mouse(x,y) movement and add
                                        // new rotation, obtained from new "delta.xy" mouse motion,
                                        // to previous one (saved in your global/static/member var)
