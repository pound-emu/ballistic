...
vkApp::setScene()
{
    ...
    mat4 clipMatrixA = mat4(1.0f,
                            ...
                            0.0f,
                            0.0f,
                            1.0f); // vulkan clip space: -y & -z

    /// imGuIZMO / vGizmo3D
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    vgTrackball.setRotation(
        quat(1, 0, 0, 0)); // vGizmo3D with NO initial rotation (default initialization)
    // track.setRotation(eulerAngleXYZ(vec3(radians(45),
    //                                      radians( 0),
    //                                      radians( 0))));  // vGizmo3D with rotation of 45 degrees
    //                                      on X axis
...
vkApp::run()
{
    ...
    { // glfwPollEvents | SDL_PollEvent ... with exit/quit check
        ...
        uboMat.mvpMatrix = clipMatrix * projMatrix * viewMatrix * compensateView * translationMatrix
                           * modelMatrix;
        uboMat.lightMatrix = clipMatrix * projMatrix * viewMatrix * compensateView
                             * translationMatrix * (static_cast<mat4>(vgTrackball.getSecondRot()))
                             * lightObj;
