...
vkAppBase::compileShaders()
{
    ...
    shaderc::SpvCompilationResult vertShaderModule = compiler.CompileGlslToSpv(
        (char *)vertCode, shaderc_glsl_vertex_shader, "vertex shader", options);
    ...
    const shaderc::SpvCompilationResult fragShaderModule = compiler.CompileGlslToSpv(
        (char *)fragCode, shaderc_glsl_fragment_shader, "fragment shader", options);
...
vkAppBase::createDescriptorsPool()
{
    ...
    descriptorPool
        = logicalDevice.createDescriptorPool(vk::DescriptorPoolCreateInfo({}, 1, 1, &poolSize));
}
...
vkAppBase::setupDescriptorsSetLayout()
{
    ...
    descriptorSetLayout = logicalDevice.createDescriptorSetLayout(
        vk::DescriptorSetLayoutCreateInfo({}, 1, &layoutBindings));
    pipelineLayout = logicalDevice.createPipelineLayout(
        vk::PipelineLayoutCreateInfo({}, 1, &descriptorSetLayout));
}

void
vkAppBase::updateDescriptorsSets(vk::Buffer &buffer, size_t bufferSize)
{
    descriptorSet = logicalDevice.allocateDescriptorSets(
    ...
    vk::WriteDescriptorSet   writeDescriptorSet(
        descriptorSet[0], 0, 0, 1, vk::DescriptorType::eUniformBuffer, {}, &desc);
...
vkAppBase::setCommandBuffer(uint32_t currentFrame)
{
    ...
    commandBuffer[currentFrame].bindDescriptorSets(
        vk::PipelineBindPoint::eGraphics, pipelineLayout, 0, 1, &descriptorSet[0], 0, nullptr);
...
vkApp::run()
{
    ...
    { // glfwPollEvents | SDL_PollEvent ... with exit/quit check
        ...
        ImGui::SetNextWindowPos(ImVec2(width - widgetSize, 0),
                                ImGuiCond_Always); // ... right aligned

        ImGui::PushStyleColor(ImGuiCol_WindowBg,
                              ImVec4(0.f, 0.f, 0.f, 0.f)); // transparent Wnd Background
        ImGui::PushStyleColor(ImGuiCol_FrameBg,
                              ImVec4(0.f, 0.f, 0.f, 0.f));         // transparent frame Background
        ...
        ImGui::Begin("##giz",
                     &isVisible,
                     ImGuiWindowFlags_NoTitleBar | // noTitle / noResize / Noscrollbar
                         ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);

        // imGuIZMO: declare global/static/member/..
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        static quat rotation(1, 0, 0, 0); // vgMath quat: default constructor initialize @ w(1) x(0)
                                          // y(0) z(0) ==> w is left/first value
        static vec3 position;

        // colored text for display quat(w,x,y,z) components
        ImGui::SetCursorPos(ImVec2(0, 0));
        ImGui::PushItemWidth(widgetSize * .25 - 2);
        ImGui::TextColored(ImVec4(1, 1, 1, 1), "w: % 1.2f", rotation.w);
        ImGui::SameLine();
        ImGui::TextColored(ImVec4(1, 0, 0, 1), "x: % 1.2f", rotation.x);
        ImGui::SameLine();
        ImGui::TextColored(ImVec4(0, 1, 0, 1), "y: % 1.2f", rotation.y);
        ImGui::SameLine();
        ...
        ImGui::gizmo3D(
            "##aaa",
            rotation,
            widgetSize); // if(ImGui::gizmo3D(...) == true) ---> widget has been updated
                         // it returns "only" a rotation (net of transformations) in base to
                         // mouse(x,y) movement and add new rotation, obtained from new "delta.xy"
                         // mouse motion, to previous one (saved in your global/static/member var)

        widgetSize *= .5; // now we use half size

        // ImGuIZMO.quat with also pan and Dolly/zoom ... and Sphere @ origin and changing
        // proportion od elements
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        imguiGizmo::resizeAxesOf(
            vec3(1, 2, 3)); // axes len, thickness axes, thickness cone (1.0 default)
        ImGui::gizmo3D(
            "##a01",
            position,
            rotation,
            widgetSize,
            imguiGizmo::sphereAtOrigin); // Ctrl+LButton = Pan ... Shift+LButton = Dolly/Zoom
        ImGui::SameLine();
        // until not restoring sizes all widgets are draw with this sizes
        imguiGizmo::restoreAxesSize(); // and is necessary restoring previous size before resize
                                       // again there is no a stack container

        // ImGuIZMO.quat same previous one, but changing central solid and axes length
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        imguiGizmo::resizeAxesOf(vec3(.5, 1, 1)); // axes len, thickness axes, thickness cone
        imguiGizmo::resizeSolidOf(1.5); // central solid size (1.0 default): 1.5 sphere bigger
        ImGui::gizmo3D(" only rotations",
                       rotation, // Display also Text
                       widgetSize,
                       imguiGizmo::sphereAtOrigin); // Ctrl+LButton = Pan ... Shift+LButton =
                                                    // Dolly/Zoom until not restoring sizes all
                                                    // widgets are draw with this sizes
        imguiGizmo::restoreAxesSize();              // restore at default axes length
                                                    // ImGuIZMO.quat
                                       //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ImGui::gizmo3D(" also Pan & Zoom",
                       position, // Display also Text
                       rotation,
                       widgetSize);     // the solid is still @ settings size
        imguiGizmo::restoreSolidSize(); // restoring at default otherwise other widgets maintain
                                        // this settings
                                        // ImGuIZMO.quat
                                        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ImGui::SameLine();
        imguiGizmo::resizeSolidOf(3.5); // central solid still bigger
        ImGui::gizmo3D("hold Ctrl/Super",
                       position,
                       rotation,
                       widgetSize);     // the solid is still @ settings size
        imguiGizmo::restoreSolidSize(); // restoring at default otherwise other widgets maintain
        ...
        mat4 translationMatrix
            = translate(mat4(1), position); // add translations (pan/dolly) to an identity matrix

        // build MVP matrix to pass to shader ==> view vkCube_05 and higher for better
        // implementation to render independent Pan/Dolly from viewMatrix
        uboMat.mvpMatrix   = clipMatrix * projMatrix * viewMatrix * translationMatrix * modelMatrix;
        uboMat.lightMatrix = clipMatrix * projMatrix * translationMatrix
                             * translate(mat4(1), lightPos) * viewMatrix
                             * scale(mat4(1), vec3(.1)); // using same cube vertex but with 10% size
