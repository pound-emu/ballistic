...
initRenderPipeline()
{
    ...
    std::array attributes { wgpu::VertexAttribute { wgpu::VertexFormat::Float32x4, 0, 0 },
                            wgpu::VertexAttribute {
                                wgpu::VertexFormat::Float32x4, sizeof(vec4), 1 } };
#else
    ...
    std::array attributes { wgpu::VertexAttribute { nullptr, wgpu::VertexFormat::Float32x4, 0, 0 },
                            wgpu::VertexAttribute {
                                nullptr, wgpu::VertexFormat::Float32x4, sizeof(vec4), 1 } };
#endif
...
updateMatrices()
{
    ...
    uboMat.lightMatrix = projMatrix * viewMatrix * compensateView * translationMatrix
                         * (static_cast<mat4>(vgizmo.getSecondRot())) * lightObj;
...
main(int, char **)
{
    ...
    // As in DAWN (internal) examples and contrarily to how they occur in wgpu_glfw_example (where
    // they are released the created objects) here don't is called glfwTerminate and glfwQuit,
    // because all objects are released after the "return"
    return 0;
}