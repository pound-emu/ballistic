...
draw()
{
    ...
    glUniformMatrix4fv(
        lghLoc, 1, false, value_ptr(lightMatrix)); // using value_ptr maintains GLM compatibility
#else
    glProgramUniformMatrix4fv(
        program, loc::mvpIdx, 1, false, value_ptr(mvpMatrix)); // vgMath permits cast to mat4*
    glProgramUniformMatrix4fv(
        program,
        loc::lightIdx,
        1,
        false,
        value_ptr(lightMatrix)); // using value_ptr maintains GLM compatibility
#endif
...
initGL()
{
    ...
    glVertexAttribPointer(
        loc::vtxIdx, nElemVtx, GL_FLOAT, GL_FALSE, 2 * nElemVtx * sizeof(float), 0L);
    ...
    glVertexArrayAttribFormat(
        vao, loc::colIdx, nElemVtx, GL_FLOAT, GL_FALSE, nElemVtx * sizeof(float));
...
mainLoop()
{
    ...
    lightMatrix = projMatrix * viewMatrix * compensateView * translationMatrix
                  * translate(mat4(1), lightPos) * scale(mat4(1), vec3(.1));
