...
glfwMouseButtonCallback(GLFWwindow *window, int button, int action, int mods)
{
    ...
    // in GLFW "CURRENTLY" button ID & vg::enums coincide, so all the switch/case statement can be
    // also omitted and pass directly "button" in "mouse" call, but is good rule always to check
    // framework IDs (in case of future changes) and select preferred vg::enum
    if (myButton >= 0)
    {
        track.mouse((vgButtons)myButton,
                    (vgModifiers)getModifier(glfwWindow),
                    action == GLFW_PRESS,
                    (int)x,
                    (int)y);
    }
...
static void
glfwMousePosCallback(GLFWwindow *window, double x, double y)
{
    if (ImGui::GetIO().WantCaptureMouse)
    {
        return;
...
main(int /* argc */, char ** /* argv */) // necessary for SDLmain in Windows
{
    ...
    while (!glfwWindowShouldClose(glfwWindow))
    {
        ...
        // click on screen
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        track.idle(); // set continuous rotation on Idle: the smooth rotation depends on speed of
                      // last mouse movements It can be adjusted from setIdleRotSpeed(1.0) > more
                      // speed, < less It can be stopped by click on screen (without mouse movement)
