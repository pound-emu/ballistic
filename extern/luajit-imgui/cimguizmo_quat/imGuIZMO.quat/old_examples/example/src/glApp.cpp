...
mainGLApp::frameInit()
{
    ...
    windowTitle += " - SDL";
    mainSDLWwnd = SDL_CreateWindow(getWindowTitle(),
...
mainGLApp::getModifier()
{
    ...
    else if (key & KMOD_SHIFT)
    {
        return KMOD_SHIFT;
    }
    else if (key & KMOD_CTRL)
    {
        return KMOD_CTRL;
    }
    else
...
glfwKeyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
    ...
    {
        if (scancode & GLFW_MOD_CONTROL)
        {
        } // CTRL+PrtScr -> request FileName
        if (!scancode)
        {
        } // CTRL+PrtScr -> TimeBased FileName
    }
...
glfwMouseButtonCallback(GLFWwindow *window, int button, int action, int mods)
{
    ...
    {
#ifndef __EMSCRIPTEN__
        if (isDoubleClick(button, action, x, y, 300))
        {
            toggleFullscreenOnOff(window);
        }
        else
...
static void
glfwCharCallback(GLFWwindow *window, unsigned int c)
{
...
bool
isDoubleClick(int button, int action, double x, double y, double ms)
{
    ...
    while (!done)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            if (event.type == SDL_WINDOWEVENT && event.window.event == SDL_WINDOWEVENT_CLOSE
                && event.window.windowID == SDL_GetWindowID(mainSDLWwnd))
            {
                done = true;
            }
            if (event.type == SDL_MOUSEBUTTONDOWN)
            {
                theWnd->onMouseButton(
                    event.button.button, SDL_MOUSEBUTTONDOWN, event.button.x, event.button.y);
            }
            if (event.type == SDL_MOUSEBUTTONUP)
            {
                theWnd->onMouseButton(
                    event.button.button, SDL_MOUSEBUTTONUP, event.button.x, event.button.y);
            }
