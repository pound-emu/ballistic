...
{
    getGizmo().setRotation(q);
}
quat
getRotation()
{
    return getGizmo().getRotation();
}

void
setPosition(const vec3 &p)
{
    getGizmo().setPosition(p);
}
vec3
getPosition()
{
    return getGizmo().getPosition();
}
#else
/////////////////////////////////////////////////////////////////////////////
...
setView()
{
    ...
    theWnd->projectionMatrix = perspective(
        radians(30.0f), (float)theApp->GetWidth() / (float)theApp->GetHeight(), 0.0f, 30.0f);
    theWnd->modelMatrix = mat4(1.0f);
    theWnd->viewMatrix  = lookAt(povVec, tgtVec, vec3(0.0f, 1.0f, 0.0f));
...
setViewOrtho()
{
    ...
    theWnd->modelMatrix      = mat4(1.0f);
    theWnd->viewMatrix = lookAt(vec3(0.f, 0.f, 1.f), vec3(0.f, 0.f, 0.f), vec3(0.f, 1.f, 0.f));

    theWnd->mvpMatrix = theWnd->projectionMatrix * theWnd->viewMatrix * theWnd->modelMatrix;
}

void
glWindow::Create(const char *wTitle, int w, int h, int xPos, int yPos)
{
}
...
glWindow::onInit()
{
...
#else
    getGizmo().setGizmoRotControl((vgButtons)GLFW_MOUSE_BUTTON_LEFT,
                                  (vgModifiers)0 /* evNoModifier */);

    getGizmo().setGizmoRotXControl((vgButtons)GLFW_MOUSE_BUTTON_LEFT, (vgModifiers)GLFW_MOD_SHIFT);
    getGizmo().setGizmoRotYControl((vgButtons)GLFW_MOUSE_BUTTON_LEFT,
                                   (vgModifiers)GLFW_MOD_CONTROL);
    getGizmo().setGizmoRotZControl((vgButtons)GLFW_MOUSE_BUTTON_LEFT,
                                   (vgModifiers)GLFW_MOD_ALT | GLFW_MOD_SUPER);

    getGizmo().setDollyControl((vgButtons)GLFW_MOUSE_BUTTON_RIGHT, (vgModifiers)0);
    getGizmo().setPanControl((vgButtons)GLFW_MOUSE_BUTTON_RIGHT,
                             (vgModifiers)GLFW_MOD_CONTROL | GLFW_MOD_SHIFT);
#endif
    // getGizmo().setPanControls(  (vgButtons) GLFW_MOUSE_BUTTON_RIGHT, (vgModifiers)
    // GLFW_MOD_SHIFT);
    ...
    float scale
        = 1.f
          / (theApp->GetWidth() > theApp->GetHeight() ? theApp->GetHeight() : theApp->GetWidth());
...
/////////////////////////////////////////////////
void
glWindow::onReshape(GLint w, GLint h)
{
    glViewport(0, 0, w, h);
    theApp->SetWidth(w);
    theApp->SetHeight(h);
...
/////////////////////////////////////////////////
void
glWindow::onKeyUp(unsigned char key, int x, int y)
{
}
...
/////////////////////////////////////////////////
void
glWindow::onSpecialKeyDown(int key, int x, int y)
{
}
...
/////////////////////////////////////////////////
void
glWindow::onKeyDown(unsigned char key, int x, int y)
{
}
...
/////////////////////////////////////////////////
void
glWindow::onSpecialKeyUp(int key, int x, int y)
{
}
...
/////////////////////////////////////////////////
void
glWindow::onMouseButton(int button, int upOrDown, int x, int y)
{
...
/////////////////////////////////////////////////
void
glWindow::onMouseWheel(int wheel, int direction, int x, int y)
{
}
...
/////////////////////////////////////////////////
void
glWindow::onPassiveMotion(int x, int y)
{
}
