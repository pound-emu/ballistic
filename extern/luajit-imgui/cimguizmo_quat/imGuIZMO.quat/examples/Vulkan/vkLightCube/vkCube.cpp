...
uint32_t
getGraphicsIndex(const vk::PhysicalDevice &physicalDevice)
{
    auto qFamProp = physicalDevice.getQueueFamilyProperties();
    return std::distance(
        qFamProp.begin(),
        std::find_if(qFamProp.begin(), qFamProp.end(), [](vk::QueueFamilyProperties const &prop) {
            return prop.queueFlags & vk::QueueFlagBits::eGraphics;
        }));
}

uint32_t
getPresentIndex(const vk::PhysicalDevice &physicalDevice, const vk::SurfaceKHR &surface)
{
...
vkAppBase::createInstance()
{
    ...
    vk::InstanceCreateInfo instInfo({},
                                    ...
                                    0,
                                    nullptr,
                                    uint32_t(framework.getExtensions().size()),
                                    framework.getExtensions().data());
...
void
vkAppBase::selectPhysicalDevice()
{
...
vkAppBase::initLogicalDevice()
{
    ...
    std::vector<uint32_t> familyIndices
        = { uniqueQueueFamilyIndices.begin(), uniqueQueueFamilyIndices.end() };
    ...
    for (uint32_t queueFamilyIndex : uniqueQueueFamilyIndices)
    {
        queueCreateInfos.push_back(
            vk::DeviceQueueCreateInfo({}, uint32_t(queueFamilyIndex), 1, &queuePriority));
    }
    ...
    const vk::DeviceCreateInfo deviceInfo(
        ...
        debug.validationLayers.size(),
        nullptr /* deprecated debug.validationLayers.data()*/, // only if Debug enabled -> check
                                                               // class
        deviceExtensions.size(),
        deviceExtensions.data());
    logicalDevice = physicalDevice.createDevice(deviceInfo);

    sm = graphQueueFamilyIdx == presentQueueFamilyIdx
             ? sharingModeStruct({ vk::SharingMode::eExclusive, 0u, nullptr })
             : sharingModeStruct({ vk::SharingMode::eConcurrent, 2u, familyIndices.data() });

    commandPool = logicalDevice.createCommandPool(vk::CommandPoolCreateInfo(
        vk::CommandPoolCreateFlagBits::eResetCommandBuffer, uint32_t(graphQueueFamilyIdx)));
...
void
vkAppBase::builSwapchain(vk::SwapchainKHR oldSwapChain)
{
...
void
vkAppBase::buildDepthBuffer()
{
    depthBuffer.init(
        physicalDevice, logicalDevice, vk::Format::eD32Sfloat, vk::Extent2D(width, height));
}

void
vkAppBase::buildImagesView()
{
    scImageView.clear();
    constexpr vk::ComponentMapping      componentMapping(vk::ComponentSwizzle::eR,
...
vkAppBase::loadSpirVShaders()
{
    ...
    vertShaderMod = logicalDevice.createShaderModule(
        vk::ShaderModuleCreateInfo({}, vertCode.size(), vertCode.data()));
    fragShaderMod = logicalDevice.createShaderModule(
        vk::ShaderModuleCreateInfo({}, fragCode.size(), fragCode.data()));
}

// Load and compile shaders, like in WebGL/OpenGL
/////////////////////////////////////////////
void
vkAppBase::compileShaders()
{
    ...
    shaderc::SpvCompilationResult vertShaderModule = compiler.CompileGlslToSpv(
        (char *)vertCode.data(), shaderc_glsl_vertex_shader, "vertex shader", options);
    ...
    const auto vertShaderCode
        = std::vector<uint32_t> { vertShaderModule.cbegin(), vertShaderModule.cend() };
    const vk::ShaderModuleCreateInfo vertShaderCreateInfo(
        {}, vertShaderCode.size() * sizeof(uint32_t), vertShaderCode.data());
    vertShaderMod = logicalDevice.createShaderModule(vertShaderCreateInfo);

    const shaderc::SpvCompilationResult fragShaderModule = compiler.CompileGlslToSpv(
        (char *)fragCode.data(), shaderc_glsl_fragment_shader, "fragment shader", options);
    ...
    const auto fragShaderCode
        = std::vector<uint32_t> { fragShaderModule.cbegin(), fragShaderModule.cend() };
    vk::ShaderModuleCreateInfo fragShaderCreateInfo(
        {}, fragShaderCode.size() * sizeof(uint32_t), fragShaderCode.data());
    fragShaderMod = logicalDevice.createShaderModule(fragShaderCreateInfo);
}

void
vkAppBase::buildRenderPass()
{
    static std::vector<vk::AttachmentDescription> attachDescriptions;

    attachDescriptions.emplace_back(vk::AttachmentDescriptionFlags(),
                                    surfaceFormat,
    ...
    vk::SubpassDescription subpassDescription(
        ...
        colorAttachmentRef,
        {},
        depthBuffer.format != vk::Format::eUndefined ? &depthAttachment : nullptr);
    renderPass = logicalDevice.createRenderPass(
        vk::RenderPassCreateInfo({}, attachDescriptions, subpassDescription));
}

void
vkAppBase::createPipelineCache()
{
...
vkAppBase::buildGraphPipeline()
{
    ...
    vk::PipelineColorBlendAttachmentState cbAttachment(false,
    ...
    vk::PipelineColorBlendStateCreateInfo colorBlending(
        {}, false, vk::LogicOp::eNoOp, cbAttachment, { { 1.0f, 1.0f, 1.0f, 1.0f } });

    vk::PipelineViewportStateCreateInfo viewportState({}, 1, {}, 1, {});

    std::vector dynamicStateFlags { vk::DynamicState::eViewport, vk::DynamicState::eScissor };
    vk::PipelineDynamicStateCreateInfo dynamicState(
        {}, dynamicStateFlags.size(), dynamicStateFlags.data());
    ...
    vk::PipelineShaderStageCreateInfo vertShaderStageInfo(
        {}, vk::ShaderStageFlagBits::eVertex, vertShaderMod, "main");
    vk::PipelineShaderStageCreateInfo fragShaderStageInfo(
        {}, vk::ShaderStageFlagBits::eFragment, fragShaderMod, "main");
...
vkAppBase::buildFramebuffer()
{
    ...
    for (size_t i = 0; i < minImageCount; i++)
    {
        ...
        scFrameBuffers[i] = logicalDevice.createFramebuffer(vk::FramebufferCreateInfo(
            {}, renderPass, isDepthPresent ? 2 : 1, imgviewBuffers, width, height, 1));
    }
}

void
vkAppBase::createDescriptorsPool()
{
    vk::DescriptorPoolSize poolSize(vk::DescriptorType::eUniformBuffer, 2);
    descriptorPool
        = logicalDevice.createDescriptorPool(vk::DescriptorPoolCreateInfo({}, 1, poolSize));
}

void
vkAppBase::setupDescriptorsSetLayout()
{
    ...
    pipelineLayout
        = logicalDevice.createPipelineLayout(vk::PipelineLayoutCreateInfo({}, descriptorSetLayout));
}

void
vkAppBase::updateDescriptorsSets(std::vector<bufferSet> &buffer)
{
    descriptorSets = logicalDevice.allocateDescriptorSets(
        vk::DescriptorSetAllocateInfo(descriptorPool, descriptorSetLayout));
    std::vector<vk::DescriptorBufferInfo> desc;
    for (auto &b : buffer)
    {
        desc.emplace_back(b.buffer, 0, b.bufferSize);
    }
...
void
vkAppBase::buildCommandBuffers()
{
    commandBuffer.resize(swapChainImages.size());
    commandBuffer = logicalDevice.allocateCommandBuffers(vk::CommandBufferAllocateInfo(
        commandPool, vk::CommandBufferLevel::ePrimary, commandBuffer.size()));
}

void
vkAppBase::setCommandBuffer(uint32_t currentFrame)
{
    ...
    const vk::Viewport viewport(
        0.0f, 0.0f, swapChainExtent.width, swapChainExtent.height, 0.0f, 1.0f);
    ...
    commandBuffer[currentFrame].beginRenderPass(
        vk::RenderPassBeginInfo(renderPass, scFrameBuffers[currentBufferIdx], rect, clearValues),
    ...
    commandBuffer[currentFrame].bindDescriptorSets(
        vk::PipelineBindPoint::eGraphics, pipelineLayout, 0, 1, &descriptorSets[0], 0, nullptr);
...
vkApp::setScene()
{
    ...
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // vgTrackball.setRotation(quat(1,0,0,0));                   // vGizmo3D with NO initial
    // rotation (DEFAULT) vgTrackball.setRotation(eulerAngleXYZ(vec3(radians(45),
    //                                     radians( 0),
    //                                     radians( 0))));      // vGizmo3D with rotation of 45
    //                                     degrees on X axis
    vgTrackball.setRotation(viewMatrix); // vGizmo3D with ViewMatrix (lookAt) rotation

    // for Pan & Dolly always bounded on screen coords (x = left/right, y = up/douw, z = in/out) we
    // remove viewMatrix rotation otherwise Pan & Dolly have as reference the Cartesian axes
    compensateView = inverse(mat4_cast(quat(viewMatrix)));

    // acquiring rotation for the light pos
    const float len = length(lightPos);
    // if(len<1.0 && len>= FLT_EPSILON) { normalize(lightPos); len = 1.0; }  // controls are not
    // necessary: lightPos is known else if(len > FLT_EPSILON)
    quat q = angleAxis(acosf(-lightPos.x / len),
                       normalize(vec3(FLT_EPSILON, lightPos.z, -lightPos.y)));
...
vkApp::draw()
{
    if (!prepareFrame())
    {
        resizeWnd();
        return;
    } // acquireNextImageKHR ==> resizeWnd on vk::OutOfDateKHRError

    const uint32_t currentFrame = currentBufferIdx; // used to differentiate in example: in reality
                                                    // they do not always coincide

    vk::detail::resultCheck(
        logicalDevice.waitForFences(
            1, &fence[currentFrame], VK_TRUE, std::numeric_limits<uint64_t>::max()),
        "waitForFences...");
    vk::detail::resultCheck(logicalDevice.resetFences(1, &fence[currentFrame]), "resetFences...");

    setCommandBuffer(currentFrame);

    constexpr vk::PipelineStageFlags waitStageMask
        = vk::PipelineStageFlagBits::eColorAttachmentOutput;
    const vk::SubmitInfo submitInfo(1,
                                    &semaphoreImageAvailable,
                                    &waitStageMask,
                                    1,
                                    &commandBuffer[currentBufferIdx],
                                    1,
                                    &semaphoreEndRendering);
    queueGraphics.submit(submitInfo, fence[currentFrame]);
    submitFrame(); // presentKHR ==> resizeWnd on vk::OutOfDateKHRError and also on
                   // vk::Result::eSuboptimalKHR with stderr message
}
...
vkApp::resizeWnd()
{
    ...
    framework.getFramebufferSize((int *)&width,
                                 (int *)&height); // e.g. glfwGetFramebufferSize / SDL_GetWindowSize
                                                  // | SDL_Vulkan_GetDrawableSize(
    rebuildAllSwapchain();                        // rebuild all swapchain components
    setPerspective();                             // recalibrate perspective aspect ratio

    // vGizmo3D: is necessary to call when resize window/surface: re-calibrate drag rotation &
    // auto-set mouse sensitivity
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    vgTrackball.viewportSize(width, height);
}

vec3
getLightPosFromQuat(quat &q, float centerDistance)
{
    return (q * vec3(-1.0f, 0.0f, 0.0f)) * centerDistance;
}
quat
getQuatRotFromVec3(vec3 &lPos)
{
    return normalize(
        angleAxis(acosf(-lPos.x / length(lPos)), normalize(vec3(FLT_EPSILON, lPos.z, -lPos.y))));
}
...
vkApp::run()
{
    ...
    { // glfwPollEvents | SDL_PollEvent ... with exit/quit check

        /// vGizmo3D: check changing button state to activate/deactivate drag movements
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        framework.checkVGizmo3DMouseEvent(vgTrackball);

        // vGizmo3D: call it every rendering loop if you want a continue rotation until you do not
        // click on screen
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        vgTrackball
            .idle(); // set continuous rotation on Idle: the slow rotation depends on speed of last
                     // mouse movement It can be adjusted from setIdleRotSpeed(1.0) > more speed, <
                     // less It can be stopped by click on screen (without mouse movement)
        vgTrackball.idleSecond(); // also for "secondary" rotation

        // ImGUI: prepare ImGUI new frame
        ImGui_ImplVulkan_NewFrame();
        framework.imguiNewFrame(); // ImGui_ImplGlfw_NewFrame | ImGui_ImplSDL2_NewFrame |
                                   // ImGui_ImplSDL3_NewFrame
        ImGui::NewFrame();

        // ImGui: Your windows here

        // using vec3 (lightPos) is necessary sync with vGizmo3D : in next example (08) this will no
        // longer be necessary
        lightPos = getLightPosFromQuat(
            vgTrackball.refSecondRot(),
            length(lightPos)); // to syncronize trackball & lightPos passed to the Widgets call

        // Render ALL ImGuIZMO_quat widgets
        renderWidgets(vgTrackball,
                      lightPos,
                      width,
                      height); // in next example (08) we will use directly quaternions

        // using vec3 (lightPos) is necessary re-sync with vGizmo3D: in next example (08) this will
        // no longer be necessary
        vgTrackball.setSecondRot(getQuatRotFromVec3(
            lightPos)); // to re-syncronize trackball & lightPos passed to the Widgets call

        // transferring the rotation to cube model matrix...
        mat4 modelMatrix = cubeObj * mat4_cast(vgTrackball.getRotation());

        // Build a "translation" matrix
        mat4 translationMatrix = translate(
            mat4(1),
            vgTrackball.getPosition()); // add translations (pan/dolly) to an identity matrix

        // old transformations used in "easy_examples": I comment and leave them to make less
        // difficult the reading of the next steps
        // uboMat.mvpMatrix   = clipMatrix * projMatrix * viewMatrix * compensateView *
        // translationMatrix * modelMatrix  ; uboMat.lightMatrix = clipMatrix * projMatrix *
        // viewMatrix * compensateView * translationMatrix *
        // (static_cast<mat4>(vgTrackball.getSecondRot())) * lightObj;

        // Decomposition of the various transformations to use (in different way) with normal, vtx
        // position and light
        ...
        uboMat.lightMatrix
            = translationMatrix * static_cast<mat4>(vgTrackball.getSecondRot()) * lightObj;

        // fill fragment uniforms:  get PointOfView (camera position) from viewMatrix...
        uboFrag.PoV = viewMatrix[3]; // in this example PoV does not change, so it would be useless
        ...
        // uboFrag.lightPos = getLightPosFromQuat(vgTrackball.refSecondRot(),length(lightPos)) +
        // vgTrackball.getPosition();

        // update uniform buffers to GPU
        for (auto &ubo : uboSceneMat)
        {
            ubo.update();
        }
...
{
    // Initialize ImGui
    const std::array pool_sizes { vk::DescriptorPoolSize(vk::DescriptorType::eCombinedImageSampler,
                                                         1) }; // array declaration C++ 17
    vk::DescriptorPoolCreateInfo pool_info(
        vk::DescriptorPoolCreateFlagBits::eFreeDescriptorSet, // flags
        1 /* .maxSets */,
        pool_sizes.size(),
        pool_sizes.data() /*, .pNext = nullptr  */);

    vk::detail::resultCheck(logicalDevice.createDescriptorPool(&pool_info, nullptr, &imguiPool),
                            "ImGui: createDescripptorPool...");
    ...
    framework.imguiInitForVulkan(
        true); // ImGui_ImplSDL2_InitForVulkan | ImGui_ImplSDL3_InitForVulkan |
               // ImGui_ImplGLFW_InitForVulkan callback ImGui active: you don't even need to handle
               // mouse events
...
void
vkApp::onInit() // called from constructor @ startup
{
    ...
    for (auto &ubo : uboSceneMat)
    { // uniform buffers for Vtx & Frag shaders
...
void
vkApp::onExit() // called from destructor @ exit
{
    // wait GPU to idle before destroy resources
    logicalDevice.waitIdle();
    // Cleanup
