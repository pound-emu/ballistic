...
#define VSYNC_ENABLED \
    true // true/false vSync on/off ==> pass from FiFo to Immediate presentation mode
...
getImageCount(const uint32_t requiredCount, const uint32_t minImgCount, const uint32_t maxImgCount)
{
    return maxImgCount > 0 ? std::min(std::max(requiredCount, minImgCount), maxImgCount)
                           : std::max(requiredCount, minImgCount);
}

class bufferSet
{
public:
    bufferSet(void *buffer, vk::DeviceSize size)
        : bufferPtr(buffer)
        , bufferSize(size)
    {
    }
    ...
    uint32_t getMemoryTypeIdx(vk::PhysicalDevice           &phyDev,
    ...
    void init(vk::PhysicalDevice     &phyDev,
              vk::Device             &device,
              vk::BufferUsageFlagBits buffFlags,
                                                 ...
                                                 | vk::MemoryPropertyFlagBits::eHostCoherent)
    {
        ...
        const vk::MemoryAllocateInfo allocInfo(
            memReq.size, getMemoryTypeIdx(phyDev, memReq.memoryTypeBits, memFlags));
    ...
    void copyToDevice(vk::DeviceSize stride = 0)
    {
        assert(logicalDevice != nullptr);
        uint8_t *deviceData
            = static_cast<uint8_t *>(logicalDevice->mapMemory(devMemBuffer, 0, bufferSize));
        if (!stride)
        {
            memcpy(deviceData, bufferPtr, bufferSize);
        }
        else
        { // TODO: not yet tested!!!!! (not used here)
            assert(0);
...
public:
              ...
              vk::ImageAspectFlags      aspectMask)
    {
        ...
        [&](uint32_t typeBits) { // findMemoryType
            ...
            {
                if ((typeBits & 1)
                    && ((physicalDevice.getMemoryProperties().memoryTypes[i].propertyFlags
                         & memoryProperties)
                        == memoryProperties))
                {
                    memoryTypeIndex = i;
        ...
        deviceMemory = device.allocateMemory(
            vk::MemoryAllocateInfo(device.getImageMemoryRequirements(image).size, memoryTypeIndex));
...
protected:
    ...
    void initSurface()
    {
        framework.getSurface(instance, physicalDevice, &surface);
    }
    ...
    { // return ImageIndex
        ...
        try
        {
            retVal = logicalDevice.acquireNextImageKHR(
                swapChain, std::numeric_limits<uint64_t>::max(), semaphoreImageAvailable, {});
        }
    ...
    void submitFrame()
    {
        const vk::PresentInfoKHR presentInfo(
            1, &semaphoreEndRendering, 1, &swapChain, &currentBufferIdx);

        vk::Result present;
        try
        {
            present = queuePresent.presentKHR(presentInfo);
        }
        ...
        {
            throw std::runtime_error("failed presentQueue...");
        }
        if (present == vk::Result::eSuboptimalKHR)
        {
            std::cerr << "Suboptimal present!" << std::endl;
            resizeWnd();
            return;
        }
    ...
    void destroySwapChainComponents()
    {
        ...
        for (const auto framebuffer : scFrameBuffers)
        {
    ...
    std::vector<std::pair<vk::Format, uint32_t>> vertexInputAttributeFormatOffset {
        { vk::Format::eR32G32B32A32Sfloat, 0 },
        { vk::Format::eR32G32B32A32Sfloat, 4 * sizeof(float) },
        { vk::Format::eR32G32B32A32Sfloat, 8 * sizeof(float) }
    };

    // preselected eImmediate with vsync=off ==> checking eMailbox is available in presentMode list:
    // minor tearing
    vk::PresentModeKHR pmType
        = VSYNC_ENABLED ? vk::PresentModeKHR::eFifo : vk::PresentModeKHR::eImmediate;
...
private:
    ...
    // I maintain this "old" matrices (from easy_examples) to better show the transformations steps
    // and UBO assignments
    ...
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    vg::vGizmo3D vgTrackball; // using vGizmo3D global/static/member instead of specifics
                              // variables... have rotations & Pan/Dolly position variables inside
                              // to use with imGuIZMO.quat And it's necessary if you want use also
                              // direct-screen manipulator
};
...
mat4 clipMatrix = mat4(1.0f,
                       ...
                       0.0f,
                       .5f,
                       1.0f); // vulkan clip space: -y & half z

// +Z set depthBufferCompareOp = vk::CompareOp::eLessOrEqual    and depthBufferClearValue = 1.0
vk::CompareOp depthBufferCompareOp
    = vk::CompareOp::eLessOrEqual;       // of vk::PipelineDepthStencilStateCreateInfo
const float depthBufferClearValue = 1.0; // "depth" value of vk::ClearDepthStencilValue

#define PERSPECTIVE \
    perspectiveRH_ZO // RightHanded space and Z-Buffer [0,1] ==> default "prespective" (both
                     // GLM/vgMath) is set to perspectiveRH_NO
#define LOOK_AT \
    lookAt // default "looAt" (both GLM/vgMath) is set to lookAtRH ==> !!! you can change ALL
           // defaults in vgMath_config or in GLM (if use it) !!!
...
vk::CompareOp depthBufferCompareOp
    = vk::CompareOp::eGreaterOrEqual;    // of vk::PipelineDepthStencilStateCreateInfo
...
#define PERSPECTIVE \
    perspectiveLH_ZO // LeftHanded space and Z-Buffer [0,1] ==> default "prespective" is set to
                     // perspectiveRH_NO Z-Buffer [-1,1]
#define LOOK_AT \
    lookAtLH // LeftHanded space ==> default "looAt" is set to lookAtRH ==> !!! you can change
             // defaults in vGizmo3D_config !!!
// the defautl values can be changed in vgConfig.h
...
// ALL these settings can be made in the code or (una tantum) set as default in vGizmo3D_config.h
// (if use it)
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

// All possible settings: These override the vGizmo3D_config.h ones: "get" current setting and
// "revert" it
