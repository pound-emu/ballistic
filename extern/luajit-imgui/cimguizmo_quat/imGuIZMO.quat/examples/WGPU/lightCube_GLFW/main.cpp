...
// I maintain this "old" matrices (from easy_examples) to better show the transformations steps and
// UBO assignments
...
}; // current undefined, but set from SurfaceCapabilities
...
void
setPerspective(int width, int height)
{
...
setScene()
{
    ...
    /// imGuIZMO / vGizmo3D
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // vgizmo.setRotation(quat(1,0,0,0));                     // vGizmo3D with NO initial rotation
    // (default initialization) vgizmo.setRotation(eulerAngleXYZ(vec3(radians(45),
    //                                     radians( 0),
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void
initVGizmo3D() // Settings to control vGizmo3D
{
    // Initialization is necessary to associate your preferences to vGizmo3D
    // These are also the DEFAULT values, so if you want to maintain these combinations you can omit
    // they and to override only the associations that you want modify
    ...
    vgizmo.setGizmoRotZControl(vg::evButton1 /* or vg::evLeftButton */,
                               vg::evAltModifier | vg::evSuperModifier);
    // Set vGizmo3D control for secondary rotation
    vgizmo.setGizmoSecondRotControl(vg::evButton2 /* or vg::evRightButton */,
                                    0 /* vg::evNoModifier */);
    ...
    // N.B. vg::enums are ONLY mnemonic: select and pass specific vg::enum to framework (that can
    // have also different IDs)

    // passing the screen sizes calibrate drag rotation and auto-set the mouse sensitivity
    vgizmo.viewportSize(
        initialWindowWidth,
        initialWindowWidth); // is necessary also to call when resize window/surface: re-calibrate
                             // drag rotation & auto-set mouse sensitivity
    // vgizmo.setGizmoFeeling(1.0);              // but if you need to more feeling with the mouse
    // use: 1.0 default,  > 1.0 more sensible, < 1.0 less sensible

    // setIdleRotSpeed(1.0)                     // If used Idle() feature (continue rotation on
    // Idle) it set that speed: more speed > 1.0 ,  less < 1.0

    // other settings if you need it
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // vgizmo.setDollyScale(1.0f);               // > 1.0 more sensible, < 1.0 less sensible
    // vgizmo.setDollyPosition(/* your pos */);  // input: float/double or vec3... in vec3 only Z is
    // acquired vgizmo.setPanScale(1.0f);                 // > 1.0 more sensible, < 1.0 less
    // sensible vgizmo.setPanyPosition(/* your pos */);   // vec3 ==> only X and Y are acquired
    // vgizmo.setPosition(/* your pos */);       // input vec3 is equivalent to call:
    // vgizmo.setDollyPosition(/* your pos */); and vgizmo.setPanyPosition(/* your pos */);
...
static void
wgpu_device_lost_callback(const wgpu::Device &,
...
static void
wgpu_error_callback(const wgpu::Device &, wgpu::ErrorType type, wgpu::StringView message)
{
...
initWGPU()
{
    ...
    auto waitedAdapterFunc { instance.RequestAdapter(
    ...
    assert(localAdapter != nullptr && waitStatus == wgpu::WaitStatus::Success
...
initRenderPipeline()
{
    ...
    std::array attributes {
        wgpu::VertexAttribute { wgpu::VertexFormat::Float32x4, 0, 0 },
    ...
    std::array attributes {
        wgpu::VertexAttribute { nullptr, wgpu::VertexFormat::Float32x4, 0, 0 },
...
void
resizeSurface(const uint32_t width, const uint32_t height)
{
    ...
    // depthTexture: overload operator "=" (assignment) Release left operand (previous object),
    // before to assign
    ...
    depthTextureDesc.usage               = wgpu::TextureUsage::RenderAttachment;
    // depthTextureView: overload operator "=" (assignment) Release left operand (previous object),
    // before to assign
...
getModifier(GLFWwindow *window)
{
    ...
    else if ((glfwGetKey(window, GLFW_KEY_LEFT_SUPER) == GLFW_PRESS)
...
void
glfwScrollCallback(GLFWwindow *window, double x, double y)
{
    vgizmo.wheel(x, y);
}
...
mouseMovement()
{
    // vGizmo3D: check changing button state to activate/deactivate drag movements (pressing
    // together left/right activate/deactivate both)
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if (!ImGui::GetIO().WantCaptureMouse)
    {
        ...
        if (glfwGetMouseButton(fwWindow, GLFW_MOUSE_BUTTON_LEFT) != leftPress)
        { // check if leftButton state is changed
            leftPress
                = leftPress == GLFW_PRESS ? GLFW_RELEASE : GLFW_PRESS; // set new (different!) state
            vgizmo.mouse(vg::evLeftButton,
                         getModifier(fwWindow), // send communication to vGizmo3D...
                         leftPress,
                         x,
                         y); // ... checking if a key modifier currently is pressed
        }
        if (glfwGetMouseButton(fwWindow, GLFW_MOUSE_BUTTON_RIGHT) != rightPress)
        { // same thing for rightButton
            rightPress = rightPress == GLFW_PRESS ? GLFW_RELEASE : GLFW_PRESS;
            vgizmo.mouse(vg::evRightButton, getModifier(fwWindow), rightPress, x, y);
        }
        // Just a trik: simulating a double press (left+right button together) using MIDDLE button,
        // sending two "consecutive" activation/deactivation calls to rotate cube and light spot
        // together
        if (glfwGetMouseButton(fwWindow, GLFW_MOUSE_BUTTON_MIDDLE) != middlePress)
        { // check if middleButton state is changed
            middlePress = middlePress == GLFW_PRESS
                              ? GLFW_RELEASE
                              : GLFW_PRESS; // set new (different!) middle button state
            vgizmo.mouse(vg::evLeftButton,
                         getModifier(fwWindow),
                         middlePress,
                         x,
                         y); // call Left activation/deactivation with same "middleStatus"
            vgizmo.mouse(vg::evRightButton,
                         getModifier(fwWindow),
                         middlePress,
                         x,
                         y); // call Right activation/deactivation with same "middleStatus"
        }
    ...
    // vGizmo3D: call it every rendering loop if you want a continue rotation until you do not click
    // on screen
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    vgizmo.idle(); // set continuous rotation on Idle: the slow rotation depends on speed of last
...
updateMatrices()
{
    ...
    mat4 translationMatrix = translate(
        mat4(1), vgizmo.getPosition()); // add translations (pan/dolly) to an identity matrix

    // old transformations used in "easy_examples": I comment and leave them to make less difficult
    // the reading of the next steps
    // uboMat.mvpMatrix   = projMatrix * viewMatrix * compensateView * translationMatrix *
    // modelMatrix  ; uboMat.lightMatrix = projMatrix * viewMatrix * compensateView *
    ...
    uboMat.PoV = viewMatrix[3]; // in this example PoV does not change, so it would be useless to
                                // update it anytime...
...
renderImGui()
{
    ...
    renderWidgets(vgizmo,
                  lightPos,
                  surfaceConfig.width,
                  surfaceConfig.height); // in next example (08) we will use directly quaternions

    // using vec3 (lightPos) is necessary re-sync with vGizmo3D: in next example (08) this will no
    // longer be necessary
    vgizmo.setSecondRot(getQuatRotFromVec3(
...
main(int, char **)
{
    glfwSetErrorCallback(
        [](int code, const char *message) { printf("GLFW Error %d: %s\n", code, message); });
    if (!glfwInit())
    {
        return -1;
    }
    ...
    fwWindow
        = glfwCreateWindow(initialWindowWidth, initialWindowHeight, appTitle, nullptr, nullptr);
    if (fwWindow == nullptr)
    {
        return -2;
    }
