...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
int
getModifier(SDL_Window *window = nullptr)
{
...
mouseMovement()
{
    ...
    if (!ImGui::GetIO().WantCaptureMouse)
    {
        ...
        if (leftPress != (mouseState & SDL_BUTTON_LMASK))
        ...
        if (rightPress != (mouseState & SDL_BUTTON_RMASK))
        {                                               // check if rightButton state is changed
        ...
        // sending two "consecutive" activation/deactivation calls to rotate obj-model and light
        // spot together
        if (middlePress != (mouseState & SDL_BUTTON_MMASK))
...
#warning "LINUX USER: Please read here..."
    // it's necessary to specify  X11 or Wayland: use "wayland" instead of "x11" or ... default x11:
    // it works also in wayland
    SDL_SetHint(SDL_HINT_VIDEODRIVER,
                "x11"); // or (outside code) export SDL_VIDEODRIVER=wayland environment variable
#endif // or    "      "    export SDL_VIDEODRIVER=$XDG_SESSION_TYPE to get the current session type
#endif
    // Init SDL
    SDL_Init(SDL_INIT_VIDEO);
    fwWindow = SDL_CreateWindow(appTitle,
