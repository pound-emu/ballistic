...
initVGizmo3D() // Settings to control vGizmo3D
{
    ...
    // track.setIdleRotSpeed(1.0)               // If used Idle() feature (continue rotation on
...
main(int /* argc */, char ** /* argv */) // necessary for SDLmain in Windows
{
    ...
    while (!done)
    {
        ...
        { // vGizmo3D: is necessary intercept mouse event not destined to ImGui
            ...
            // both activate/deacivate both functionality)
            //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            {                                              // check if leftButton state is changed
                ...
                track.mouse(vg::evLeftButton,
                            getModifier(sdlWindow), // send communication to vGizmo3D...
                            leftPress,
                            x,
            ...
            // sending two "consecutive" activation/deactivation to rotate cube and light spot
            // together
            if (middlePress != (mouseState & SDL_BUTTON_MMASK))
            { // check if middleButton state is changed
                middlePress
                    = mouseState & SDL_BUTTON_MMASK; // set new (different!) middle button state
                track.mouse(vg::evRightButton,
                            getModifier(sdlWindow),
                            middlePress,
                            x,
        ...
        track.idle(); // set continuous rotation on Idle: the slow rotation depends on speed of last
                      // mouse movements It can be adjusted from setIdleRotSpeed(1.0) > more speed,
