...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
vg::vGizmo3D
    track; // using vGizmo3D global/static/member instead of specifics variables...
...
draw()
{
    ...
    glDrawArraysInstanced(
        GL_TRIANGLES, 0, nVertex, 2); // now using instanced draw to "simulate" light
...
void
glfwWindowSizeCallback(GLFWwindow *window, int w, int h)
{
    width  = w;
    height = h;
    setPerspective();
...
initGL()
{
    ...
    glVertexAttribPointer(loc::nrmIdx,
                          nElemVtx,
                          GL_FLOAT,
                          GL_FALSE,
                          sizeof(cubePNC[0]),
                          (void *)(nElemVtx * sizeof(float)));
    glEnableVertexAttribArray(loc::nrmIdx);

    glVertexAttribPointer(loc::colIdx,
                          nElemVtx,
                          GL_FLOAT,
                          GL_FALSE,
                          sizeof(cubePNC[0]),
                          (void *)(2 * nElemVtx * sizeof(float)));
...
initVGizmo3D() // Settings to control vGizmo3D
{
    ...
    track.setGizmoRotZControl(vg::evButton1 /* or vg::evLeftButton */,
                              vg::evAltModifier | vg::evSuperModifier);
    // Set vGizmo3D control for secondary rotation
    track.setGizmoSecondRotControl(vg::evButton2 /* or vg::evRightButton */,
                                   0 /* vg::evNoModifier */);
    ...
    track.setPanControl(vg::evButton2 /* or vg::evRightButton */,
                        vg::evShiftModifier | vg::evSuperModifier);
    ...
    // track.setGizmoFeeling(1.0);              // 1.0 default,  > 1.0 more sensible, < 1.0 less
    // sensible

    // setIdleRotSpeed(1.0)                     // If used Idle() feature (continue rotation on
    // Idle) it set that speed: more speed > 1.0 ,  less < 1.0 track.setWheelScale(1.0); // scale on
    // wheel rotation:  less < 1.0 < more

    // other settings if you need it
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // track.setDollyScale(1.0f);               // > 1.0 more sensible, < 1.0 less sensible
    // track.setDollyPosition(/* your pos */);  // input: float/double or vec3... in vec3 only Z is
    // acquired track.setPanScale(1.0f);                 // > 1.0 more sensible, < 1.0 less sensible
    // track.setPanyPosition(/* your pos */);   // vec3 ==> only X and Y are acquired
    // track.setPosition(/* your pos */);       // input vec3 is equivalent to call:
    // track.setDollyPosition(/* your pos */); and track.setPanyPosition(/* your pos */);
...
int
main(int /* argc */, char ** /* argv */) // necessary for SDLmain in Windows
{
    initFramework(); // initialize GLFW framework
    initGL();        // init OpenGL building vaoBuffer and shader program (compile and link vtx/frag
                     // shaders)
    ...
    imguiGizmo::setGizmoFeelingRot(
        1.5f); // default 1.0, >1 more mouse sensitivity, <1 less mouse sensitivity
...
mainLoop()
{
    ...
    if (!ImGui::GetIO().WantCaptureMouse)
    {
        ...
        if (glfwGetMouseButton(glfwWindow, GLFW_MOUSE_BUTTON_LEFT) != leftPress)
        { // check if leftButton state is changed
            leftPress
                = leftPress == GLFW_PRESS ? GLFW_RELEASE : GLFW_PRESS; // set new (different!) state
            track.mouse(vg::evLeftButton,
                        getModifier(glfwWindow), // send communication to vGizmo3D...
                        leftPress,
                        x,
                        y); // ... checking if a key modifier currently is pressed
        }
        if (glfwGetMouseButton(glfwWindow, GLFW_MOUSE_BUTTON_RIGHT) != rightPress)
        { // same thing for rightButton
            rightPress = rightPress == GLFW_PRESS ? GLFW_RELEASE : GLFW_PRESS;
            track.mouse(vg::evRightButton, getModifier(glfwWindow), rightPress, x, y);
        }
    ...
    // on screen
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    track.idle(); // set continuous rotation on Idle: the slow rotation depends on speed of last
    ...
    renderWidgets(
        track, lightPos, width, height); // in next example (08) we will use directly quaternions

    // using vec3 (lightPos) is necessary re-sync with vGizmo3D: in next example (08) this will no
    // longer be necessary
    track.setSecondRot(getQuatRotFromVec3(
    ...
    mat4 translationMatrix = translate(
        mat4(1), track.getPosition()); // add translations (pan/dolly) to an identity matrix
