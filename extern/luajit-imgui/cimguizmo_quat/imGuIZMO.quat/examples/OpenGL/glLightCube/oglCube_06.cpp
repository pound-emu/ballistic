...
initGL()
{
    ...
    glVertexArrayAttribFormat(
        vao, loc::nrmIdx, nElemVtx, GL_FLOAT, GL_FALSE, nElemVtx * sizeof(float));
    glEnableVertexArrayAttrib(vao, loc::nrmIdx);

    glVertexArrayAttribBinding(vao, loc::colIdx, 0);
    glVertexArrayAttribFormat(
        vao, loc::colIdx, nElemVtx, GL_FLOAT, GL_FALSE, 2 * nElemVtx * sizeof(float));
...
initVGizmo3D() // Settings to control vGizmo3D
{
    ...
    track.viewportSize(width, height); // is necessary also to call when resize window/surface:
                                       // re-calibrate drag rotation & auto-set mouse sensitivity
    // track.setGizmoFeeling(1.0);              // but if you need to more feeling with the mouse
    ...
    while (!glfwWindowShouldClose(glfwWindow))
    {
        ...
        if (!ImGui::GetIO().WantCaptureMouse)
        {
            ...
            { // check if leftButton state is changed
                leftPress = leftPress == GLFW_PRESS ? GLFW_RELEASE
                                                    : GLFW_PRESS; // set new (different!) state
            ...
            // Just a trik: simulating a double press (left+right button together) using MIDDLE
            // button, sending two "consecutive" activation/deactivation calls to rotate cube and
            // light spot together
        ...
        ImGui::TextColored(ImVec4(1, 1, 1, 1), "w: % 1.2f", track.getRotation().w);
        ImGui::SameLine();
        ImGui::TextColored(ImVec4(1, 0, 0, 1), "x: % 1.2f", track.getRotation().x);
        ImGui::SameLine();
        ImGui::TextColored(ImVec4(0, 1, 0, 1), "y: % 1.2f", track.getRotation().y);
        ImGui::SameLine();
        ...
        uboMat.PoV = viewMatrix[3]; // in this example PoV does not change, so it would be useless
