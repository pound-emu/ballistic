...
void
TextDiff::SetText(const std::string_view &left, const std::string_view &right)
{
    ...
    decoratorCallback = [this](TextEditor::Decorator &decorator) { this->decorateLine(decorator); };
...
void
TextDiff::SetLanguage(const Language *l)
{
    language = l;
    colorizer.updateEntireDocument(leftDocument, language);
    colorizer.updateEntireDocument(rightDocument, language);
...
void
TextDiff::Render(const char *title, const ImVec2 &size, bool border)
{
    if (sideBySideMode)
    {
...
void
TextDiff::splitLines(std::vector<std::string_view> &result, const std::string_view &text)
{
...
void
TextDiff::decorateLine(TextEditor::Decorator &decorator)
{
    ...
    switch (line.status)
    {
        ...
        case LineStatus::added:
            ImGui::TextColored(
                color, " %*s %*d +", leftLineNumberDigits, "", rightLineNumberDigits, lineRight);
            break;

        case LineStatus::deleted:
            ImGui::TextColored(
                color, " %*d %*s -", leftLineNumberDigits, lineLeft, rightLineNumberDigits, "");
            break;
    }
}
...
void
TextDiff::renderSideBySide(const char *title, const ImVec2 &size, bool border)
{
    ...
    if (scrollToLineNumber >= 0)
    {
        ...
        switch (scrollToAlignment)
        {
            case Scroll::alignTop:
                ImGui::SetNextWindowScroll(ImVec2(
                    0.0f, std::max(0.0f, static_cast<float>(scrollToLineNumber) * glyphSize.y)));
                break;
            ...
            case Scroll::alignMiddle:
                ImGui::SetNextWindowScroll(
                    ImVec2(0.0f,
                           std::max(0.0f,
                                    static_cast<float>(scrollToLineNumber - visibleLines / 2)
                                        * glyphSize.y)));
                break;
    ...
    textColumnWidth
        = static_cast<float>(visibleSize.x - leftLineNumberWidth - rightLineNumberWidth) / 2.0f;
    ...
    lastVisibleLine  = std::min(
        static_cast<int>(std::floor((ImGui::GetScrollY() + visibleSize.y) / glyphSize.y)),
    ...
    for (auto i = firstVisibleLine; i <= lastVisibleLine; i++)
    {
        ...
        switch (line.status)
        {
            case LineStatus::common:
                ...
                drawList->AddText(
                    ImVec2(leftLineNumberPos, y), palette.get(Color::lineNumber), buffer);
                snprintf(buffer, sizeof(buffer), " %*d", rightLineNumberDigits, lineRight);
                drawList->AddText(
                    ImVec2(rightLineNumberPos, y), palette.get(Color::lineNumber), buffer);
                break;
            ...
            case LineStatus::added:
                ...
                drawList->AddRectFilled(
                    ImVec2(rightTextPos, y), ImVec2(rightTextEnd, y + glyphSize.y), addedColor);
...
void
TextDiff::renderSideBySideLine(float x, float y, TextEditor::Line &line)
{
...
TextDiff::renderSideBySideTextScrollbars()
{
    auto maxColumnsWidth
        = std::max(leftDocument.getMaxColumn(), rightDocument.getMaxColumn()) * glyphSize.x;
    ...
    if (maxColumnsWidth > visibleColumnsWidth)
    {
        ...
        auto textScrollbarTop
            = std::max(outerRect.Min.y + borderSize, outerRect.Max.y - borderSize - scrollbarSize);
        ImRect leftScrollbarFrame(
            leftTextPos, textScrollbarTop, rightLineNumberPos, textScrollbarTop + scrollbarSize);
        ImRect rightScrollbarFrame(
            rightTextPos, textScrollbarTop, rightTextEnd, textScrollbarTop + scrollbarSize);
        ImS64 scroll = static_cast<ImS64>(textScroll);

        if (ImGui::ScrollbarEx(leftScrollbarFrame,
        ...
        if (ImGui::ScrollbarEx(rightScrollbarFrame,
        ...
        {
            textScroll = std::clamp(textScroll - ImGui::GetIO().MouseWheelH * ImGui::GetFontSize(),
                                    0.0f,
            ...
            {
                textScroll
                    = std::min(textScroll + glyphSize.x, maxColumnsWidth - visibleColumnsWidth);
            }
    ...
    if (ImGui::IsWindowHovered())
    {
        ...
        else if (ImGui::IsKeyPressed(ImGuiKey_DownArrow))
        {
            ...
            {
                ImGui::SetScrollY(
                    std::min(ImGui::GetScrollY() + glyphSize.y, ImGui::GetScrollMaxY()));
            }
        }
        else if (ImGui::IsKeyPressed(ImGuiKey_PageUp))
        {
            ImGui::SetScrollY(
                std::max(ImGui::GetScrollY() - (visibleLines - 2) * glyphSize.y, 0.0f));
        }
        else if (ImGui::IsKeyPressed(ImGuiKey_PageDown))
        {
            ImGui::SetScrollY(std::min(ImGui::GetScrollY() + (visibleLines - 2) * glyphSize.y,
                                       ImGui::GetScrollMaxY()));
        }
    }
}
