...
breakpoints from deleted lines. The callbacks can be turned off by calling (**SetInsertor** and **SetDeletor**) again
with nullptr as the argument. If the callbacks are in place when loading text, you'll get a callback for each line. If
