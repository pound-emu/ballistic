...
Editor::openFile()
{
    if (isDirty())
    {
        showConfirmClose([this]() { showFileOpen(); });
    }
    else
    {
        showFileOpen();
...
void
Editor::openFile(const std::string &path)
{
    try
    {
...
Editor::renderMenuBar()
{
    ...
    if (ImGui::BeginMenuBar())
    {
        ...
        if (ImGui::BeginMenu("Edit"))
        {
            ...
            if (ImGui::MenuItem(
                    "Paste", " " SHORTCUT "V", nullptr, ImGui::GetClipboardText() != nullptr))
            {
                editor.Paste();
            }
            ...
            ImGui::Separator();
            bool flag;
            flag = editor.IsInsertSpacesOnTabs();
            if (ImGui::MenuItem("Insert Spaces on Tabs", nullptr, &flag))
            {
                editor.SetInsertSpacesOnTabs(flag);
            };
            ...
            if (ImGui::MenuItem("Spaces To Tabs", nullptr, nullptr, !editor.IsInsertSpacesOnTabs()))
            {
                editor.SpacesToTabs();
            }
        ...
        if (ImGui::BeginMenu("Selection"))
        {
            ...
            if (ImGui::MenuItem("Add Next Occurrence",
                                " " SHORTCUT "D",
                                nullptr,
                                editor.CurrentCursorHasSelection()))
            {
        ...
        if (ImGui::BeginMenu("View"))
        {
            ...
            bool flag;
            if (ImGui::MenuItem("Autocomplete", nullptr, &autocomplete))
            {
            ...
            flag = editor.IsShowSpacesEnabled();
            if (ImGui::MenuItem("Show Spaces", nullptr, &flag))
            {
                editor.SetShowSpacesEnabled(flag);
            };
            flag = editor.IsShowTabsEnabled();
            if (ImGui::MenuItem("Show Tabs", nullptr, &flag))
            {
                editor.SetShowTabsEnabled(flag);
            };
...
{
    // language support
    static const char *languages[] = { "C",    "C++",  "Cs",   "AngelScript", "Lua", "Python",
                                       "GLSL", "HLSL", "JSON", "Markdown",    "SQL" };

    static const TextEditor::Language *definitions[]
        = { TextEditor::Language::C(),    TextEditor::Language::Cpp(),
            TextEditor::Language::Cs(),   TextEditor::Language::AngelScript(),
            TextEditor::Language::Lua(),  TextEditor::Language::Python(),
            TextEditor::Language::Glsl(), TextEditor::Language::Hlsl(),
            TextEditor::Language::Json(), TextEditor::Language::Markdown(),
    ...
    auto statusSize = std::snprintf(status,
                                    sizeof(status),
...
void
Editor::showConfirmClose(std::function<void()> callback)
{
...
void
Editor::showError(const std::string &message)
{
...
Editor::renderDiff()
{
    ...
    if (ImGui::BeginPopupModal(
            "Changes since Opening File##diff", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
...
Editor::renderConfirmClose()
{
    ...
    if (ImGui::BeginPopupModal("Confirm Close", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        static constexpr float buttonWidth = 80.0f;
        ImGui::Indent(ImGui::GetContentRegionAvail().x - buttonWidth * 2.0f
...
void
Editor::setAutocompleteMode(bool flag)
{
    ...
    if (flag)
    {
        ...
        // if you want live tracking, change the 3000 to 0 (performance hit will be minimal for
        // small documents)
        editor.SetChangeCallback([this]() { buildAutocompleteTrie(); }, 3000);
    }
    else
    {
...
Editor::buildAutocompleteTrie()
{
    ...
    editor.IterateIdentifiers([this](const std::string &identifier) { trie.insert(identifier); });
}
