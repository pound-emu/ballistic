...
static ImVec2
add(const ImVec2 &a, const ImVec2 &b)
{
    return ImVec2(a.x + b.x, a.y + b.y);
}

static ImVec2
sub(const ImVec2 &a, const ImVec2 &b)
{
    return ImVec2(a.x - b.x, a.y - b.y);
}

static ImVec2
mul(const ImVec2 &a, float s)
{
    return ImVec2(a.x * s, a.y * s);
}

static ImVec2
mul(const ImVec2 &a, const ImVec2 &b)
{
    return ImVec2(a.x * b.x, a.y * b.y);
}

static float
lengthSq(const ImVec2 &v)
{
...
static ImVec2
rotate(const ImVec2 &p, float radians)
{
...
static ImVec2
snapToAngleIncrement(const ImVec2 &v, float incrementRadians)
{
...
static ImVec2
alignedOppositeHandle(const ImVec2 &handle, float oppositeLength)
{
...
static bool
hasCurveBetween(const Anchor &a, const Anchor &b)
{
...
static void
drawControlPoint(
    ImDrawList *drawList, const ImVec2 &center, float radius, ControlPointShape shape, ImU32 color)
{
    switch (shape)
    {
        ...
        case ControlPointShape::Square:
            drawList->AddRectFilled(
                sub(center, ImVec2(radius, radius)), add(center, ImVec2(radius, radius)), color);
            break;
...
static void
drawSegment(ImDrawList   *drawList,
            const Config &config,
            const ImVec2 &origin,
            const Anchor &a,
            const Anchor &b,
...
bool
MakeAligned(Anchor &anchor)
{
    return alignHandles(anchor);
}

bool
MakeMirrored(Anchor &anchor)
{
    if (!anchor.hasHandleIn && !anchor.hasHandleOut)
    {
...
bool
AddHandles(Anchor &anchor, float handleLength)
{
    if (anchor.hasHandleIn && anchor.hasHandleOut)
    {
        return false;
...
bool
DeleteHandles(Anchor &anchor)
{
    if (!anchor.hasHandleIn && !anchor.hasHandleOut && anchor.handleMode == HandleMode::Corner)
    {
        return false;
    }
...
ImVec2
Transform::LocalToCanvas(const ImVec2 &local) const
{
...
ImVec2
Transform::CanvasToLocal(const ImVec2 &canvas) const
{
    const float safeZoom = std::max(zoom, kMinZoom);
...
Result
Editor::Draw(const char *id, Path &path, const Config &config)
{
    Result result;
    ...
    {
        drawList->AddRectFilled(
            boxSelectStartScreen_, boxSelectCurrentScreen_, IM_COL32(112, 174, 255, 35));
        drawList->AddRect(
            boxSelectStartScreen_, boxSelectCurrentScreen_, IM_COL32(112, 174, 255, 170));
    }
...
bool
Editor::IsAnchorSelected(int anchor) const
{
    return std::find(selectedAnchors_.begin(), selectedAnchors_.end(), anchor)
           != selectedAnchors_.end();
}

void
Editor::SelectAnchor(int anchor, HitPart part, bool addToSelection)
{
...
void
Editor::RemoveSelectedAnchors(Path &path)
{
    if (selectedAnchors_.empty())
    {
        return;
...
void
Editor::BeginAnchorDrag(const Path &path, const ImVec2 &mouseLocal)
{
...
void
Editor::UpdateBoxSelection(Path         &path,
                           const Config &config,
                           ...
                           bool          addToSelection)
{
    ...
    for (int i = 0; i < static_cast<int>(path.anchors.size()); ++i)
    {
        ...
        if (anchorScreen.x >= minX && anchorScreen.x <= maxX && anchorScreen.y >= minY
            && anchorScreen.y <= maxY && !IsAnchorSelected(i))
        {
...
void
Editor::BeginEdit(const Config &config, EditKind kind, int anchorIndex)
{
    if (editActive_)
    {
        return;
...
void
Editor::EndEdit(const Config &config)
{
    if (!editActive_)
    {
        return;
...
Editor::Hit
Editor::HitTest(const Path   &path,
                const Config &config,
                const ImVec2 &canvasOrigin,
...
ImVec2
Editor::LocalToScreen(const Config &config, const ImVec2 &canvasOrigin, const ImVec2 &local) const
{
    return add(canvasOrigin, config.transform.LocalToCanvas(local));
}

ImVec2
Editor::ScreenToLocal(const Config &config, const ImVec2 &canvasOrigin, const ImVec2 &screen) const
{
    return config.transform.CanvasToLocal(sub(screen, canvasOrigin));
}

void
Editor::DrawGrid(ImDrawList   *drawList,
                 const ImVec2 &origin,
                 const ImVec2 &size,
...
void
Editor::DrawPath(ImDrawList   *drawList,
                 const Path   &path,
                 const Config &config,
                 const ImVec2 &canvasOrigin) const
{
    ...
    {
        const bool activeClickDragSegment = dragging_ && newAnchorDrag_ && selectedAnchor_ == i;
        drawSegment(drawList,
                    config,
                    canvasOrigin,
                    path.anchors[i - 1],
                    path.anchors[i],
    ...
    if (path.closed && path.anchors.size() > 2)
    {
        drawSegment(drawList,
                    ...
                    path.anchors.back(),
                    path.anchors.front(),
                    config.style.pathColor,
                    config.style.pathThickness);
    }
}

void
Editor::DrawControls(ImDrawList   *drawList,
                     const Path   &path,
                     const Config &config,
                     const ImVec2 &canvasOrigin,
                     const Hit    &hoveredHit) const
{
    for (int i = 0; i < static_cast<int>(path.anchors.size()); ++i)
    {
        ...
        const bool   closeTarget = i == 0 && hoveredHit.anchor == 0
                                   && hoveredHit.part == HitPart::Anchor && path.anchors.size() >= 3
                                   && !path.closed;
        const ImVec2 anchorScreen = LocalToScreen(config, canvasOrigin, anchor.position);
        ...
        if (anchor.hasHandleIn)
        {
            const ImVec2 handle
                = LocalToScreen(config, canvasOrigin, add(anchor.position, anchor.handleIn));
            drawList->AddLine(anchorScreen,
                              handle,
                              config.style.handleLineColor,
                              config.style.handleLineThickness);
            drawControlPoint(drawList,
                             handle,
                             config.style.handleRadius,
        ...
        drawControlPoint(drawList,
                         anchorScreen,
                         config.style.anchorRadius + 1.5f,
                         config.style.anchorShape,
                         IM_COL32(0, 0, 0, 180));
        drawControlPoint(drawList,
                         anchorScreen,
                         config.style.anchorRadius,
                         config.style.anchorShape,
                         anchorColor);
    }
}
...
bool
Editor::HandleKeyboard(Path &path, const Config &config, Result &result)
{
    ...
    else if (ImGui::IsKeyPressed(ImGuiKey_Backspace) || ImGui::IsKeyPressed(ImGuiKey_Delete))
    {
        ...
        {
            BeginEdit(config, EditKind::DeleteAnchor, static_cast<int>(path.anchors.size()) - 1);
            path.anchors.pop_back();
            selectedAnchor_ = path.anchors.empty() ? -1 : static_cast<int>(path.anchors.size()) - 1;
            selectedPart_   = selectedAnchor_ >= 0 ? HitPart::Anchor : HitPart::None;
            if (path.anchors.empty())
            {
                penActive_ = false;
        ...
        else if (selectedAnchor_ >= 0 && selectedAnchor_ < static_cast<int>(path.anchors.size()))
        {
            BeginEdit(config, EditKind::DeleteAnchor, selectedAnchor_);
            path.anchors.erase(path.anchors.begin() + selectedAnchor_);
            selectedAnchor_ = std::min(selectedAnchor_, static_cast<int>(path.anchors.size()) - 1);
            selectedPart_   = selectedAnchor_ >= 0 ? HitPart::Anchor : HitPart::None;
            changed         = true;
            EndEdit(config);
        }
                    ...
                    Result       &result)
{
    ...
    if ((hovered_ || active_) && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left)
        && hoveredHit.anchor >= 0 && hoveredHit.anchor < static_cast<int>(path.anchors.size())
    ...
    if (hovered_ && ImGui::IsMouseClicked(ImGuiMouseButton_Left))
    {
        ...
        {
            if (penActive_ && hoveredHit.anchor == 0 && hoveredHit.part == HitPart::Anchor
                && path.anchors.size() >= 3)
            {
    ...
    if (dragging_ && ImGui::IsMouseDragging(ImGuiMouseButton_Left, 0.0f) && selectedAnchor_ >= 0
        ...
        if (selectedPart_ == HitPart::Anchor)
        {
                     ...
                     && dragStartAnchorPositions_.size() == selectedAnchors_.size())
            {
                ...
                for (int i = 0; i < static_cast<int>(selectedAnchors_.size()); ++i)
                {
                    ...
                    if (anchorIndex >= 0 && anchorIndex < static_cast<int>(path.anchors.size()))
                    {
                        path.anchors[anchorIndex].position
                            = add(dragStartAnchorPositions_[i], delta);
                    }
                }
                changed = true;
            }
        ...
        else if (selectedPart_ == HitPart::HandleIn)
        {
            ...
            else if (anchor.handleMode == HandleMode::Aligned)
            {
                const float outLength
                    = anchor.hasHandleOut ? length(anchor.handleOut) : length(anchor.handleIn);
        ...
        else if (selectedPart_ == HitPart::HandleOut)
        {
            ...
            else if (anchor.handleMode == HandleMode::Aligned)
            {
                const float inLength
                    = anchor.hasHandleIn ? length(anchor.handleIn) : length(anchor.handleOut);
