...
public:
    ...
    {
        return wantsMouseCapture_;
    }
    bool WantsKeyboardCapture() const
    {
        return wantsKeyboardCapture_;
    }

    void ClearSelection();
    int  GetSelectedAnchor() const
    {
        return selectedAnchor_;
    }
    int GetSelectedAnchorCount() const
    {
        return static_cast<int>(selectedAnchors_.size());
    }
    HitPart GetSelectedPart() const
    {
        return selectedPart_;
    }
...
private:
    ...
    Hit    HitTest(const Path   &path,
                   const Config &config,
                   const ImVec2 &canvasOrigin,
                   const ImVec2 &mouseScreen) const;
    ImVec2 LocalToScreen(const Config &config,
                         const ImVec2 &canvasOrigin,
                         const ImVec2 &local) const;
    ImVec2 ScreenToLocal(const Config &config,
                         const ImVec2 &canvasOrigin,
    ...
    void DrawPath(ImDrawList   *drawList,
                  const Path   &path,
                  const Config &config,
    ...
    void UpdateBoxSelection(Path         &path,
                            const Config &config,
                            const ImVec2 &canvasOrigin,
                            bool          addToSelection);
